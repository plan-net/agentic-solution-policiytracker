    ...
    try:
        def _inspect():
            with kb.driver.session(database=kb.database) as session:
                ...
                """)
                for record in result:
                    print(f"  {record['from_type']} -[{record['relationship']}]-> {record['to_type']} ({record['count']} times)")
                
                print("\n=== SAMPLE NODES ===")
                result = session.run("MATCH (n) RETURN labels(n)[0] as type, n.name as name LIMIT 10")
                for record in result:

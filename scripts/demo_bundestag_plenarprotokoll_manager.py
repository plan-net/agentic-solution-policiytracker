"""Demo script for BundestagPlenarprotokoll Manager Skill.

This script demonstrates the complete workflow:
1. MCP Server operations for Plenarprotokoll
2. CRUD Subagent parallel execution
3. Manager Skill intelligent sync

Run with: uv run python scripts/demo_bundestag_plenarprotokoll_manager.py
"""
import asyncio
import logging
import sys
...
async def demo_mcp_server():
    """Demo 1: MCP Server direct operations for Plenarprotokoll."""
    print("\n" + "=" * 80)
    print("DEMO 1: MCP Server - Direct CRUD Operations for Plenarprotokoll")
    print("=" * 80)
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Create Plenarprotokoll node
            print("\n2. Create Plenarprotokoll Node")
            create_payload = {
                "entity_type": "Plenarprotokoll",
                "properties": {
                    "plenarprotokoll_id": "demo_test_plenarprotokoll_001",
            ...
            # Update Plenarprotokoll node
            print("\n3. Update Plenarprotokoll Node")
            update_payload = {
                "entity_type": "Plenarprotokoll",
                "node_id": "demo_test_plenarprotokoll_001",
                "properties": {"sitzungsnummer": "124"},
            }
            ...
            # Query Plenarprotokoll nodes
            print("\n4. Query Plenarprotokoll Nodes")
            query_payload = {
                "entity_type": "Plenarprotokoll",
                "filters": {"sitzungsnummer": "123"},
                "limit": 10,
            }
            ...
            # Delete node (soft)
            print("\n5. Delete Plenarprotokoll Node (Soft)")
            delete_payload = {
                "entity_type": "Plenarprotokoll",
                "node_id": "demo_test_plenarprotokoll_001",
                "hard_delete": False,
...
async def demo_crud_subagent():
    """Demo 2: CRUD Subagent parallel execution for Plenarprotokoll."""
    print("\n" + "=" * 80)
    print("DEMO 2: CRUD Subagent - Parallel Execution for Plenarprotokoll")
    print("=" * 80)
    ...
    try:
        ...
        # Single operation
        print("\n2. Execute Single Plenarprotokoll Operation")
        single_op = {
            "operation": "create_node",
            "entity_type": "Plenarprotokoll",
            "properties": {
                "plenarprotokoll_id": "demo_single_plenarprotokoll_001",
                "titel": "Einzelner Test Plenarprotokoll",
                "sitzungsnummer": "125",
                "datum": "2024-02-20",
                "wahlperiode": 20,
                "active": True,
        ...
        print("\n3. Execute 20 Plenarprotokoll Operations in Parallel")
        parallel_ops = []
        for i in range(20):
            parallel_ops.append(
                {
                    ...
                    "entity_type": "Plenarprotokoll",
                    "properties": {
                        "plenarprotokoll_id": f"demo_parallel_plenarprotokoll_{i:03d}",
                        "titel": f"Parallel Plenarprotokoll {i}",
                        "sitzungsnummer": "123",
                        "datum": "2024-03-10",
                        "wahlperiode": 20,
                        "active": True,
            ...
            cleanup_ops.append(
                {
                    ...
                    "entity_type": "Plenarprotokoll",
                    "node_id": f"demo_parallel_plenarprotokoll_{i:03d}",
                    "hard_delete": True,
                }
            )
        cleanup_ops.append(
            {
                "operation": "delete_node",
                "entity_type": "Plenarprotokoll",
                "node_id": "demo_single_plenarprotokoll_001",
                "hard_delete": True,
            }
        )
...
async def demo_manager_skill():
    """Demo 3: Manager Skill intelligent sync for Plenarprotokoll."""
    print("\n" + "=" * 80)
    print("DEMO 3: Manager Skill - Intelligent Plenarprotokoll Sync")
    print("=" * 80)

    try:
        from src.skills.bundestag_plenarprotokoll_manager import BundestagPlenarprotokollManager

        # Initialize manager with mock DIP client
        print("\n1. Initialize Plenarprotokoll Manager (Mock DIP Client)")
        manager = BundestagPlenarprotokollManager(use_mock_dip=True)
        print("   ✅ Manager initialized")

        # Check sync status
        print("\n2. Check Plenarprotokoll Sync Status")
        status = await manager.check_sync_status()
        print(f"   Status: {status['status']}")
        print(f"   Total differences: {status['total_differences']}")
        print(f"   Missing Plenarprotokolln: {status['missing_plenarprotokollen']}")
        print(f"   Outdated Plenarprotokolln: {status['outdated_plenarprotokollen']}")
        ...
        if status["total_differences"] > 0:
            ...
            dry_result = await manager.sync_all_plenarprotokollen(dry_run=True)
            ...
            # Actual sync
            print("\n4. Perform Plenarprotokoll Sync")
            sync_result = await manager.sync_all_plenarprotokollen()
        ...
        # Specific Plenarprotokoll sync
        print("\n5. Sync Specific Plenarprotokolln")
        specific_result = await manager.sync_specific_plenarprotokollen(
            ["287654", "289123"]
        )
...
async def run_all_demos():
    ...
    print("\n" + "=" * 80)
    print("BundestagPlenarprotokoll Manager - Complete Demo")
    print("=" * 80)
    print(
        "\nThis demo will test the complete CRUD system for Plenarprotokoll:"
    )
    print("  1. MCP Server - Direct database operations for Plenarprotokoll")
    print("  2. CRUD Subagent - Parallel execution with Ray")
    print("  3. Manager Skill - Intelligent Plenarprotokoll data synchronization")
    print("\n" + "=" * 80)
    ...
    print("=" * 80)
    print("\nNext Steps:")
    print("  1. Review Plenarprotokoll documentation in docs/")
    ...
    print("\n✅ All Plenarprotokoll demos completed successfully!")

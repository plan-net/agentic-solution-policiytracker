    ...
    try:
        ...
        entity_prompt_variables = {
            ...
            "relationships": json.dumps(
                [{"type": "AFFECTS", "from": "GDPR", "to": "Tech Companies"}], indent=2
            ),
        ...
        try:
            entity_prompt = await prompt_manager.get_prompt(
                "graphrag_entity_analysis", variables=entity_prompt_variables
            )
        ...
        relationship_prompt_variables = {
            ...
            "relationship_patterns": json.dumps(
                [{"type": "IMPLEMENTS", "strength": "high"}], indent=2
            ),
        ...
        try:
            relationship_prompt = await prompt_manager.get_prompt(
                "graphrag_relationship_insights", variables=relationship_prompt_variables
            )
            print(
                f"   ✅ GraphRAG relationship insights prompt loaded ({len(relationship_prompt)} chars)"
            )
        except Exception as e:
        ...
        comparison_prompt_variables = {
            ...
            "traditional_results": json.dumps(
                {"method": "keyword_scoring", "score": 0.75}, indent=2
            ),
        ...
        try:
            comparison_prompt = await prompt_manager.get_prompt(
                "graphrag_comparative_analysis", variables=comparison_prompt_variables
            )
            print(
                f"   ✅ GraphRAG comparative analysis prompt loaded ({len(comparison_prompt)} chars)"
            )
        except Exception as e:

"""Demo script for BundestagPerson Manager Skill.
...
Run with: uv run python scripts/demo_bundestag_person_manager.py
"""
import asyncio
import logging
import sys
...
async def demo_mcp_server():
    """Demo 1: MCP Server direct operations."""
    print("\n" + "=" * 80)
    print("DEMO 1: MCP Server - Direct CRUD Operations")
    print("=" * 80)
...
async def demo_crud_subagent():
    ...
    print("\n" + "=" * 80)
    print("DEMO 2: CRUD Subagent - Parallel Execution")
    print("=" * 80)
    ...
    try:
        ...
        print("\n3. Execute 20 Operations in Parallel")
        parallel_ops = []
        for i in range(20):
            parallel_ops.append(
                {
                    ...
                    "entity_type": "BundestagPerson",
                    "properties": {
                        "person_id": f"demo_parallel_{i:03d}",
                        "vorname": f"Person{i}",
                        "nachname": "Parallel",
                        "active": True,
                    },
            ...
            cleanup_ops.append(
                {
                    ...
                    "entity_type": "BundestagPerson",
                    "node_id": f"demo_parallel_{i:03d}",
                    "hard_delete": True,
                }
            )
        cleanup_ops.append(
...
async def demo_manager_skill():
    ...
    print("\n" + "=" * 80)
    print("DEMO 3: Manager Skill - Intelligent Sync")
    print("=" * 80)
    ...
    try:
        ...
        print("\n1. Initialize Manager (Mock DIP Client)")
        ...
        if status["total_differences"] > 0:
            ...
            # Actual sync
            print("\n4. Perform Sync")
            sync_result = await manager.sync_all_persons()
        ...
        # Specific person sync
        print("\n5. Sync Specific Persons")
        specific_result = await manager.sync_specific_persons(
            ["11004809", "11003142"]
        )
...
async def run_all_demos():
    ...
    print("\n" + "=" * 80)
    print("BundestagPerson Manager - Complete Demo")
    print("=" * 80)
    print(
        "\nThis demo will test the complete CRUD system:"
    )
    print("  1. MCP Server - Direct database operations")
    print("  2. CRUD Subagent - Parallel execution with Ray")
    print("  3. Manager Skill - Intelligent data synchronization")
    print("\n" + "=" * 80)

"""
Test multilingual search for Consumer Credit Directive (Direct OpenAI API).

This script tests whether searching in English vs German yields the same results
after ada-002 migration. Uses OpenAI API directly (no APISIX gateway).

Usage:
    python scripts/test_multilingual_search_ccd_direct.py
    python scripts/test_multilingual_search_ccd_direct.py --top-k 20
"""
...
async def get_embedding(client: AsyncOpenAI, text: str) -> List[float]:
    """Get ada-002 embedding directly from OpenAI."""
...
async def vector_search_entities(
    ...
    # Get embedding
    query_embedding = await get_embedding(client, query_text)
...
def print_comparison_table(
    ...
    if only_english:
        ...
        for item in english_results:
            if item[key] in only_english and count < 3:
                count += 1
                item_name = item.get('name', item.get('fact', item.get('preview', 'N/A')))
                print(f"  {count}. {item_name[:70]} (sim: {item['similarity']:.3f})")
    ...
    if only_german:
        ...
        for item in german_results:
            if item[key] in only_german and count < 3:
                count += 1
                item_name = item.get('name', item.get('fact', item.get('preview', 'N/A')))
                print(f"  {count}. {item_name[:70]} (sim: {item['similarity']:.3f})")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test multilingual search for Consumer Credit Directive (Direct OpenAI)",
    ...
    print(f"Embedding Model: text-embedding-ada-002 (Direct OpenAI API)")
        ...
        async with driver.session(database=settings.NEO4J_DATABASE) as session:
            ...
            if not args.relationships_only and not args.episodic_only:
                ...
                english_entities = await vector_search_entities(
                    session, openai_client, english_query, args.threshold, args.top_k
                )
                print(f"   Found {len(english_entities)} results")

                print(f"\n🔍 Searching entities with German query...")
                german_entities = await vector_search_entities(
                    session, openai_client, german_query, args.threshold, args.top_k
                )
                print(f"   Found {len(german_entities)} results")
            ...
            if not args.entities_only and not args.episodic_only:
                ...
                print(f"   Found {len(english_rels)} results")

                print(f"\n🔍 Searching relationships with German query...")
                german_rels = await vector_search_relationships(
                    session, openai_client, german_query, args.threshold, args.top_k
                )
                print(f"   Found {len(german_rels)} results")
            ...
            if not args.entities_only and not args.relationships_only:
                ...
                print(f"   Found {len(english_eps)} results")

                print(f"\n🔍 Searching episodic content with German query...")
                german_eps = await vector_search_episodic(
                    session, openai_client, german_query, args.threshold, args.top_k
                )
                print(f"   Found {len(german_eps)} results")

        ...
        async with kb:
            ...
            if stats["chunks"] > 0:
                ...
                search_results = await kb.search_documents(
                    query="GDPR privacy data protection", top_k=3
                )
                ...
                graph_results = await kb.search_with_graph_traversal(
                    query="AI Act regulation implementation", top_k=3
                )

                log_info("Graph traversal results:")
                ...
                if graph_results["results"]:
                    for i, result in enumerate(graph_results["results"], 1):
                        ...
                        log_info(
                            f"  Regulation count: {political_context.get('regulation_count', 0)}"
                        )

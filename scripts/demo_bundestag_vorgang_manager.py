"""Demo script for BundestagVorgang Manager Skill.

This script demonstrates the complete workflow:
1. MCP Server operations for Vorgang
2. CRUD Subagent parallel execution
3. Manager Skill intelligent sync

Run with: uv run python scripts/demo_bundestag_vorgang_manager.py
"""
import asyncio
import logging
import sys
...
async def demo_mcp_server():
    """Demo 1: MCP Server direct operations for Vorgang."""
    print("\n" + "=" * 80)
    print("DEMO 1: MCP Server - Direct CRUD Operations for Vorgang")
    print("=" * 80)
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Create Vorgang node
            print("\n2. Create Vorgang Node")
            create_payload = {
                "entity_type": "Vorgang",
                "properties": {
                    "vorgang_id": "demo_test_vorgang_001",
            ...
            # Update Vorgang node
            print("\n3. Update Vorgang Node")
            update_payload = {
                "entity_type": "Vorgang",
                "node_id": "demo_test_vorgang_001",
            ...
            # Query Vorgang nodes
            print("\n4. Query Vorgang Nodes")
            query_payload = {
                "entity_type": "Vorgang",
                "filters": {"vorgangstyp": "Gesetzgebung"},
                "limit": 10,
            }
...
async def demo_crud_subagent():
    """Demo 2: CRUD Subagent parallel execution for Vorgang."""
    print("\n" + "=" * 80)
    print("DEMO 2: CRUD Subagent - Parallel Execution for Vorgang")
    print("=" * 80)
    ...
    try:
        ...
        # Single operation
        print("\n2. Execute Single Vorgang Operation")
        single_op = {
            "operation": "create_node",
            "entity_type": "Vorgang",
            "properties": {
                "vorgang_id": "demo_single_vorgang_001",
                "titel": "Einzelner Test Vorgang",
                "vorgangstyp": "Antrag",
                "beratungsstand": "In Beratung",
                "wahlperiode": 20,
                "active": True,
        ...
        print("\n3. Execute 20 Vorgang Operations in Parallel")
        parallel_ops = []
        for i in range(20):
            parallel_ops.append(
                {
                    ...
                    "entity_type": "Vorgang",
                    "properties": {
                        "vorgang_id": f"demo_parallel_vorgang_{i:03d}",
                        "titel": f"Parallel Vorgang {i}",
                        "vorgangstyp": "Gesetzgebung",
                        "beratungsstand": "In Beratung",
                        "wahlperiode": 20,
                        "active": True,
            ...
            cleanup_ops.append(
                {
                    ...
                    "entity_type": "Vorgang",
                    "node_id": f"demo_parallel_vorgang_{i:03d}",
                    "hard_delete": True,
                }
            )
        cleanup_ops.append(
            {
                "operation": "delete_node",
                "entity_type": "Vorgang",
                "node_id": "demo_single_vorgang_001",
                "hard_delete": True,
            }
        )
...
async def demo_manager_skill():
    """Demo 3: Manager Skill intelligent sync for Vorgang."""
    print("\n" + "=" * 80)
    print("DEMO 3: Manager Skill - Intelligent Vorgang Sync")
    print("=" * 80)

    try:
        from src.skills.bundestag_vorgang_manager import BundestagVorgangManager

        # Initialize manager with mock DIP client
        print("\n1. Initialize Vorgang Manager (Mock DIP Client)")
        manager = BundestagVorgangManager(use_mock_dip=True)
        print("   ✅ Manager initialized")

        # Check sync status
        print("\n2. Check Vorgang Sync Status")
        status = await manager.check_sync_status()
        print(f"   Status: {status['status']}")
        print(f"   Total differences: {status['total_differences']}")
        print(f"   Missing Vorgänge: {status['missing_vorgaenge']}")
        print(f"   Outdated Vorgänge: {status['outdated_vorgaenge']}")
        ...
        if status["total_differences"] > 0:
            ...
            dry_result = await manager.sync_all_vorgaenge(dry_run=True)
            ...
            # Actual sync
            print("\n4. Perform Vorgang Sync")
            sync_result = await manager.sync_all_vorgaenge()
        ...
        # Specific Vorgang sync
        print("\n5. Sync Specific Vorgänge")
        specific_result = await manager.sync_specific_vorgaenge(
            ["287654", "289123"]
        )
...
async def run_all_demos():
    ...
    print("\n" + "=" * 80)
    print("BundestagVorgang Manager - Complete Demo")
    print("=" * 80)
    print(
        "\nThis demo will test the complete CRUD system for Vorgang:"
    )
    print("  1. MCP Server - Direct database operations for Vorgang")
    print("  2. CRUD Subagent - Parallel execution with Ray")
    print("  3. Manager Skill - Intelligent Vorgang data synchronization")
    print("\n" + "=" * 80)
    ...
    print("=" * 80)
    print("\nNext Steps:")
    print("  1. Review Vorgang documentation in docs/")
    ...
    print("\n✅ All Vorgang demos completed successfully!")

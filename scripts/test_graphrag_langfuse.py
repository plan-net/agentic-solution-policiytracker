...
async def test_graphrag_langfuse_integration():
    ...
    try:
        async with builder:
            ...
            print(f"   Political entities found: {graph_results.get('political_entities_found', 0)}")
            ...
            if "error" in entity_analysis:
                ...
                if key_actors:
                    ...
                    for actor in key_actors[:2]:  # Show first 2
                        print(f"     - {actor.get('name', 'Unknown')}: {actor.get('type', 'Unknown type')}")
            ...
            if network_metrics:
                ...
                print(f"   Relationship types: {len(network_metrics.get('relationship_types', []))}")
            ...
            if "error" in relationship_insights:
                ...
                if recommendations:
                    ...
                    for rec in recommendations[:2]:  # Show first 2
                        print(f"     - {rec.get('priority', 'Unknown')}: {rec.get('description', 'No description')}")
            ...
            if "error" in comparison:
                ...
                if unique_insights:
                    ...
                    for insight in unique_insights[:2]:  # Show first 2
                        print(f"     - {insight.get('insight_type', 'Unknown')}: {insight.get('strategic_value', 'Unknown value')}")

...
class TestCompleteMultiAgentWorkflow:
    ...
    async def complete_orchestrator(self):
        ...
        mock_tools = {
                ...
                return_value={
                    "nodes": [
                        ...
                        },
                        {
                            ...
                            "type": "Organization",
                            "summary": "EU executive body",
                        },
                        {
                            "name": "Artificial Intelligence",
                            "type": "Technology",
                            "summary": "AI technology domain",
                        },
                    ],
                    "edges": [
                        {
            ...
            "get_entity_relationships": AsyncMock(
                return_value={
                    "relationships": [
                        {
            ...
            "get_entity_timeline": AsyncMock(
                return_value={
                    "timeline": [
                        {
                            "date": "2024-05-21",
                            "event": "European Parliament adoption",
                            "significance": "high",
                        },
                        {
                            "date": "2024-07-12",
                            "event": "Council of EU final approval",
                            "significance": "high",
                        },
                        {
                            "date": "2024-08-01",
                            "event": "Publication in Official Journal",
                            "significance": "critical",
                        },
                        {
                            "date": "2025-02-02",
                            "event": "Governance provisions apply",
                            "significance": "medium",
                        },
                    ]
                }
        ...
        mock_tool_integration.get_tool_recommendations.return_value = [
            {
                "tool_name": "search",
                "parameters": {
                    "query": "EU AI Act European Commission",
                    "limit": 10,
        ...
        mock_tool_integration.execute_tool_sequence.return_value = [
            {
                ...
                "insights": [
                    "EU AI Act is comprehensive legislation",
    ...
    async def test_complete_workflow_information_seeking(self, complete_orchestrator):
        ...
        result = await complete_orchestrator.process_query(
            query=query, session_id=session_id, user_id=user_id, stream_callback=stream_callback
        )
    ...
    async def test_complete_workflow_with_personalization(self, complete_orchestrator):
        ...
        complete_orchestrator.memory_manager.analyze_user_conversation_patterns = AsyncMock(
            return_value=mock_user_patterns
        )
        complete_orchestrator.memory_manager.get_personalization_recommendations = AsyncMock(
            return_value={
                "thinking_speed": "fast",
        ...
        result = await complete_orchestrator.process_query(query=query, user_id=user_id)

        # Validate personalization was applied
        assert result["success"] is True

        # Verify personalization methods were called
        complete_orchestrator.memory_manager.get_personalization_recommendations.assert_called_once_with(
            user_id
        )
    ...
    async def test_streaming_orchestrator_real_time(self):
        ...
        event_types = [event.get("type") for event in streaming_events]
        assert any(
            event_type in ["workflow_update", "final_result", "error"] for event_type in event_types
        )
    ...
    async def test_error_handling_and_recovery(self, complete_orchestrator):
        ...
        mock_llm.ainvoke.side_effect = [
            ...
            MagicMock(
                content='{"intent": "information_seeking", "confidence": 0.8, "complexity": "simple", "primary_entity": "Test", "secondary_entities": [], "key_relationships": [], "temporal_scope": "current", "analysis_strategy": "focused", "search_priorities": [], "expected_information_types": [], "memory_insights": {}}'
            ),  # Second call succeeds
            MagicMock(
                content='{"strategy_type": "simple", "estimated_execution_time": 5.0, "tool_sequence": [], "success_criteria": {"primary": "test"}, "backup_strategies": [], "optimization_notes": {}}'
            ),
            MagicMock(
                content='{"response_text": "Error recovery response", "confidence_level": 0.6, "information_completeness": "partial", "source_count": 1, "key_insights_count": 1, "response_metadata": {}, "follow_up_suggestions": [], "synthesis_notes": {}}'
            ),
        ]

        # Test that system handles errors gracefully
        result = await complete_orchestrator.process_query(
            query="Test error handling", session_id="error_test_session"
        )
    ...
    async def test_tool_integration_intelligence(self, complete_orchestrator):
        ...
        for test_case in test_queries:
            ...
            result = await complete_orchestrator.process_query(
                query=test_case["query"], session_id=f"tool_test_{test_case['expected_intent']}"
            )
            ...
            call_args = (
                complete_orchestrator.tool_integration_manager.get_tool_recommendations.call_args
            )
            if call_args:
    ...
    async def test_memory_persistence_and_learning(self, complete_orchestrator):
        ...
        for i, query in enumerate(queries):
            ...
            result = await complete_orchestrator.process_query(
                query=query, user_id=user_id, session_id=f"learning_session_{i}"
            )

            interaction_data.append(
                {"query": query, "confidence": result["confidence"], "success": result["success"]}
            )

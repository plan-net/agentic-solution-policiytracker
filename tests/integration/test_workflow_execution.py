...
from src.workflow.graph import create_workflow, create_checkpointer, execute_workflow
...
class TestWorkflowExecution:
    ...
    def _create_test_state(self, job_request: JobRequest, job_id: str = "test_job_001") -> WorkflowState:
    ...
    @pytest.mark.asyncio
    async def test_full_graph_execution_local(self, test_config, test_documents_dir, checkpoint_db, local_settings):
        ...
        with patch('src.workflow.nodes.ContentProcessor') as mock_processor:
            with patch('src.workflow.nodes.HybridScoringEngine') as mock_engine:
                with patch('src.workflow.nodes.ReportGenerator') as mock_generator:
                    # Setup mocks
                    mock_processor.return_value.process_document = AsyncMock(return_value=self._mock_processed_content())
                    mock_engine.return_value.score_document_hybrid = AsyncMock(return_value=self._mock_scoring_result())
                    mock_generator.return_value.prepare_report_data = AsyncMock(return_value={"test": "data"})
    ...
    async def test_full_graph_execution_azure(self, mock_azure_client_class, test_config, mock_azure_settings, mock_azure_storage_client):
        ...
        with patch('src.workflow.nodes.ContentProcessor') as mock_processor:
            with patch('src.workflow.nodes.HybridScoringEngine') as mock_engine:
                with patch('src.workflow.nodes.ReportGenerator') as mock_generator:
                    ...
                    mock_engine.return_value.score_document_hybrid = AsyncMock(return_value=self._mock_scoring_result())
                    mock_generator.return_value.prepare_report_data = AsyncMock(return_value={"test": "data"})
                    mock_generator.return_value.generate_markdown_content = AsyncMock(return_value="# Test Report")
    ...
    @pytest.mark.asyncio
    async def test_workflow_checkpointing_behavior(self, test_config, test_documents_dir, checkpoint_db):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_workflow_executions(self, test_config, test_documents_dir, checkpoint_db):
        ...
        tasks = [
            run_workflow(f"concurrent-thread-{i}")
            for i in range(3)
        ]
    ...
    async def test_azure_checkpointing_integration(self, mock_azure_client_class, test_config, mock_azure_settings, mock_azure_storage_client):
...
class TestLangGraphPatterns:
    ...
    def test_graph_structure_snapshot(self):
        ...
        assert set(nodes) == set(expected_nodes), f"Graph nodes mismatch. Expected: {expected_nodes}, Got: {nodes}"

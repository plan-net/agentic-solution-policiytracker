...
class TestETLIntegration:
    ...
    async def test_policy_landscape_collector_end_to_end(
        self, client_context_file, sample_exa_response, tmp_path
    ):
        """Test complete policy collection flow."""

        collector = PolicyLandscapeCollector(
            exa_api_key="test_api_key", client_context_path=client_context_file
        )
    ...
    async def test_etl_collector_factory_integration(self):
        ...
        exa_collector = create_collector(collector_type="exa_direct", api_key="test_key")
        ...
        for collector_type in collectors:
            try:
                collector = create_collector(collector_type=collector_type, api_key="test_key")
    ...
    async def test_error_handling_and_recovery(self, client_context_file):
        ...
        with patch("pathlib.Path.mkdir"), patch("builtins.open", create=True):
            result = await collector.collect_policy_documents(
                days_back=7, max_results_per_query=2, max_concurrent_queries=1
            )
    ...
    def test_etl_configuration_validation(self):
        ...
        invalid_configs = [{"collector_type": "invalid"}, {"days_back": -1}, {"max_items": 0}]

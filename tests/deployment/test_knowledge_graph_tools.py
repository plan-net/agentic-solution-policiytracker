...
class TestKnowledgeGraphTools:
    ...
    async def test_tool_recommendations_integration(self, tool_manager):
        ...
        for case in test_cases:
            ...
            ), f"Recommendations should be list for {case['intent']}"
            ...
            if "expected_tools" in case:
                ...
                for expected_tool in case["expected_tools"]:
                    assert (
                        expected_tool in recommended_tool_names
                    ), f"Missing expected tool {expected_tool} in {recommended_tool_names}"

            if "expected_min_tools" in case:
                assert (
                    len(recommendations) >= case["expected_min_tools"]
                ), f"Too few recommendations for {case['intent']}"
    ...
    async def test_tool_execution_performance(self, tool_manager):
        ...
        for test in performance_tests:
            ...
            try:
                ...
                assert (
                    execution_time < test["max_time"]
                ), f"{test['tool_name']} too slow: {execution_time:.2f}s"
    ...
    async def test_invalid_tool_parameters(self):
        ...
        for case in invalid_cases:
            try:
                ...
                assert isinstance(
                    recommendations, list
                ), "Should return list even for invalid inputs"

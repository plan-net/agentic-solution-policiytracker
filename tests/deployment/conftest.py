...
from src.chat.server.app import ChatServer
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", 
        "deployment: mark test as deployment test (requires real services)"
    )
    config.addinivalue_line(
        "markers",
        "slow: mark test as slow (takes more than 10 seconds)"
    )
    config.addinivalue_line(
        "markers",
        "requires_data: mark test as requiring knowledge graph data"
    )

...
class TestMultiAgentWorkflow:
    ...
    async def test_query_understanding_agent(self, http_client, deployment_config, sample_queries):
        ...
        for category, queries in sample_queries.items():
            ...
            for query in queries[:1]:  # Test one query per category
                ...
                assert "query_understanding" in workflow_stages, f"Query understanding stage missing for: {query}"
    ...
    async def test_agent_sequence_validation(self, http_client, deployment_config):
        ...
        expected_sequence = ["query_understanding", "tool_planning"]  # At minimum these two should execute
        ...
        for i, expected_agent in enumerate(expected_sequence):
            ...
            assert workflow_sequence[i] == expected_agent, f"Wrong agent order. Expected {expected_agent}, got {workflow_sequence[i] if i < len(workflow_sequence) else 'None'}"
    ...
    async def test_response_quality_metrics(self, http_client, deployment_config, quality_metrics):
        ...
        for query in test_queries:
            ...
            async with http_client.post(
                ...
                has_structured_info = any(marker in content.lower() for marker in ["gdpr", "regulation", "compliance", "act", "directive"])
        ...
        for scenario in error_scenarios:
            ...
            try:
                async with http_client.post(
                    ...
                    assert response.status in [200, 400], f"Unexpected status {response.status} for {scenario['name']}"
                    
                    if response.status == 200:
                        data = await response.json()
                        assert "choices" in data, f"Malformed response for {scenario['name']}"
                        assert len(data["choices"]) > 0, f"No choices in response for {scenario['name']}"
    ...
    async def test_concurrent_requests(self, http_client, deployment_config):
        ...
        if successful_requests:
            assert avg_response_time < 90, f"Response time too slow under load: {avg_response_time:.2f}s"
...
class TestWorkflowStateValidation:
    ...
    async def test_session_isolation(self, http_client, deployment_config):
        ...
        assert session_responses[0]["response_id"] != session_responses[1]["response_id"], "Session IDs should be different"
        
        # Content should be different for different queries
        content_similarity = len(set(session_responses[0]["content"]) & set(session_responses[1]["content"])) / max(len(session_responses[0]["content"]), len(session_responses[1]["content"]))

...
async def mock_graphiti_client():
    ...
    async def mock_search(query, limit=5):
        ...
        return {
            ...
            "edges": [
                {"source": "European Commission", "target": "EU AI Act", "type": "PROPOSED"}
            ],
            "facts": [
...
async def test_understand_query_node():
    ...
    from src.chat.agent.nodes import understand_query_node, initialize_nodes
    from src.chat.agent.state import QueryAnalysis
    ...
    with patch('src.chat.agent.nodes.prompt_manager') as mock_pm:
        mock_pm.get_prompt = AsyncMock(return_value="""

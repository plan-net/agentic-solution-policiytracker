...
class TestETLInterfaceContracts:
    ...
    def test_storage_interface_contract(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            required_methods = ['save_document', 'get_document', 'list_documents', 
                              'document_exists', 'delete_document']
            
            for method_name in required_methods:
                assert hasattr(storage, method_name), \
                    f"LocalStorage must have {method_name} method"
...
class TestPolicyLandscapeCollector:
    ...
    @pytest.mark.asyncio
    async def test_policy_collection_real_integration(self, client_context_file, test_exa_api_response):

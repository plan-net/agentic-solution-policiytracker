...
def mock_apify_response():
    ...
    return {"data": {"defaultDatasetId": "test_dataset_id"}}
...
class TestExaDirectCollector:
    ...
    async def test_collect_news_success(self, collector, mock_exa_response):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post:
            ...
            # Test collection
            results = await collector.collect_news(query="EU regulation", max_items=10, days_back=7)
...
class TestApifyNewsCollector:
    ...
    async def test_collect_news_success(self, collector, mock_apify_response, mock_apify_items):
        ...
        with patch("aiohttp.ClientSession.post") as mock_post, patch(
            ...
            # Test collection
            results = await collector.collect_news(query="Apple", max_items=10, days_back=7)
...
class TestPolicyLandscapeCollector:
    ...
    def collector(self, tmp_path):
        ...
        return PolicyLandscapeCollector(
            exa_api_key="test_key", client_context_path=str(context_file), storage_type="local"
        )
    ...
    async def test_collect_policy_documents_success(self, collector, mock_exa_response):
        ...
        collector.exa_collector.collect_news = AsyncMock(
            return_value={"success": True, "articles": mock_exa_response["results"]}
        )
...
class TestETLIntegration:
    ...
    async def test_policy_collector_with_real_query_generation(self, tmp_path):
        ...
        client_context = {
            ...
            "regulatory_focus": {"topic_patterns": ["AI regulation", "data privacy"]},
        }
        ...
        collector = PolicyLandscapeCollector(
            exa_api_key="test_key", client_context_path=str(context_file)
        )

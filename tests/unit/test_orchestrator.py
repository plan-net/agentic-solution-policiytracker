...
from src.chat.agent.base import MultiAgentState, AgentRole
...
class TestMultiAgentOrchestrator:
    ...
    def mock_llm(self):
        ...
        def mock_response(messages):
            ...
            if "analyze" in content.lower():
                return MagicMock(content='{"intent": "information_seeking", "confidence": 0.95, "complexity": "simple", "primary_entity": "EU AI Act", "secondary_entities": [], "key_relationships": [], "temporal_scope": "current", "analysis_strategy": "focused", "search_priorities": [], "expected_information_types": [], "memory_insights": {}}')
            elif "plan" in content.lower():
                return MagicMock(content='{"strategy_type": "focused", "estimated_execution_time": 20.0, "tool_sequence": [{"step": 1, "tool_name": "entity_lookup", "parameters": {"entity_name": "EU AI Act"}, "purpose": "test", "estimated_time": 5.0, "dependency": null}], "success_criteria": {"primary": "test"}, "backup_strategies": [], "optimization_notes": {}}')
            elif "synthesize" in content.lower():
                return MagicMock(content='{"response_text": "Test response about EU AI Act", "confidence_level": 0.9, "information_completeness": "high", "source_count": 1, "key_insights_count": 3, "response_metadata": {"word_count": 100}, "follow_up_suggestions": [], "synthesis_notes": {}}')
            else:
    ...
    def mock_tools(self):
        ...
        return {
            "entity_lookup": mock_entity_lookup,
            "search": mock_search
        }
    ...
    async def test_execution_time_calculation(self, orchestrator):
        ...
        state_total = MultiAgentState(
            original_query="test",
            execution_metadata={"total_execution_time": 25.5},
            messages=[]
        )
        ...
        state = MultiAgentState(
            ...
            tool_results=[
                ...
                },
                {
                    ...
                    "source_citations": ["Source 2", "Source 3"]
                }
            ],
            messages=[]
        )
    ...
    async def test_process_query_basic(self, orchestrator):
        ...
        with patch.object(orchestrator.streaming_manager, 'emit_thinking') as mock_thinking, \
             patch.object(orchestrator.streaming_manager, 'emit_agent_transition') as mock_transition, \
             patch.object(orchestrator.streaming_manager, 'emit_progress') as mock_progress:
...
class TestOrchestratorIntegration:
    ...
    async def test_memory_learning_integration(self):
        ...
        mock_llm = AsyncMock(spec=BaseLLM)
        mock_llm.ainvoke.return_value = MagicMock(content='{"intent": "information_seeking", "confidence": 0.85, "complexity": "simple", "primary_entity": "Test", "secondary_entities": [], "key_relationships": [], "temporal_scope": "current", "analysis_strategy": "focused", "search_priorities": [], "expected_information_types": [], "memory_insights": {}}')
        ...
        with patch.object(orchestrator.memory_manager, 'learn_from_query') as mock_learn, \
             patch.object(orchestrator.memory_manager, 'update_tool_performance') as mock_tool_perf:

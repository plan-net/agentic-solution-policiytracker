...
class TestBuildAgentContext:
    ...
    def test_basic_context_building(self):
        ...
        context = build_agent_context(
            ...
            session_state={"original_query": "What is GDPR?"}
        )
        ...
        for agent in expected_agents:
            if agent in available_agents:  # Only test if file exists
                ...
                try:
                    ...
                    assert ("What is the EU AI Act?" in rendered or 
                           "information_seeking" in rendered or
                           "medium" in rendered)  # Check for query or analysis content
                    assert "{{" not in rendered  # No unprocessed template variables
    ...
    async def test_query_understanding_prompt_rendering(self):
        ...
        context = {
            ...
            "conversation_history": [
                "Previous question about GDPR compliance"
            ],
            "learned_patterns": {
                "common_intent": "enforcement_tracking"
            }
        }

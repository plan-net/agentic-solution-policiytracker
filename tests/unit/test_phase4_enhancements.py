...
from src.chat.agent.streaming import StreamingThinkingGenerator, StreamingManager
    ...
    async def test_contextual_thinking_emission(self, thinking_generator):
        ...
        with patch.object(thinking_generator.streaming_manager, 'emit_thinking') as mock_emit:
            ...
            mock_emit.assert_called_once_with(
                "query_understanding",
                "Analyzing EU AI Act with 95% confidence"
            )
...
class TestToolIntegration:
    ...
    def test_tool_recommendations_relationship_analysis(self, tool_integration_manager):
        ...
        tool_names = [r["tool_name"] for r in recommendations]
        assert "find_paths_between_entities" in tool_names or "get_entity_relationships" in tool_names
    ...
    async def test_tool_execution_sequence(self, tool_integration_manager):
        ...
        mock_tool._arun = AsyncMock(return_value={"test": "result", "nodes": [{"name": "Test Entity"}]})
...
class TestIntegratedOrchestrator:
    ...
    async def test_enhanced_learning_integration(self, enhanced_orchestrator):
        ...
        final_state = MultiAgentState(
            ...
            execution_metadata={
                "agent_execution_times": {"search": 2.0}
            },
    ...
    async def test_personalization_setup(self, enhanced_orchestrator):
        ...
        with patch.object(enhanced_orchestrator, '_learn_from_interaction') as mock_learn, \
             patch.object(enhanced_orchestrator.graph, 'astream') as mock_stream:
            ...
            enhanced_orchestrator.memory_manager.get_personalization_recommendations.assert_called_once_with("test_user")
            assert enhanced_orchestrator.thinking_generator.user_preferences["thinking_speed"] == "detailed"

...
class TestBatchDocumentLoaderAzure:
    ...
    async def test_discover_files_azure(self, mock_azure_client_class, mock_azure_storage_client):
        ...
        mock_azure_storage_client.list_blobs.assert_called_once_with(
            "input-documents", prefix="jobs/test_job/input/"
        )
    ...
    async def test_get_file_stats_azure(self, mock_azure_client_class, mock_azure_storage_client):
        ...
        # Get stats
        blob_names = [
            "jobs/test/input/doc1.txt",
            "jobs/test/input/doc2.md",
            "jobs/test/input/doc3.pdf",
        ]
...
class TestContentProcessorAzure:
    ...
    async def test_cache_content_azure(
        self, mock_azure_client_class, mock_azure_storage_client, sample_processed_content
    ):
        ...
        # Cache content
        result = await processor._cache_content(
            "test_file.txt", "test_doc", sample_processed_content, "test_job"
        )
    ...
    @pytest.mark.asyncio
    @patch("src.processors.content_processor.AzureStorageClient")
    async def test_get_cached_content_azure(
        self, mock_azure_client_class, mock_azure_storage_client, sample_processed_content
    ):

...
def sample_workflow_state(sample_job):
    ...
    return WorkflowState(job_id=sample_job.id, job_request=sample_job.request)
...
class TestWorkflowState:
    ...
    def test_workflow_state_initialization(self, sample_job):
        ...
        state = WorkflowState(job_id=sample_job.id, job_request=sample_job.request)
...
class TestWorkflowExecution:
    ...
    async def test_execute_workflow_with_resume(self, mock_create_checkpointer, sample_job):
        ...
        mock_compiled = AsyncMock()
        final_state = WorkflowState(job_id=sample_job.id, job_request=sample_job.request)
        mock_compiled.ainvoke.return_value = final_state

        with patch.object(workflow, "compile", return_value=mock_compiled):
            result = await execute_workflow(
                workflow, sample_job, resume_from_checkpoint="checkpoint_123"
            )
...
class TestWorkflowMetrics:
    ...
    async def test_workflow_metrics_calculation(
        self, sample_job, test_documents, test_scoring_results
    ):

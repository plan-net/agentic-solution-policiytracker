...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers",
        "ray: mark test as requiring Ray cluster (local mode)"
    )
    config.addinivalue_line(
        "markers",
        "slow: mark test as slow running (performance tests)"
    )
...
def mock_ray_tasks():
    ...
    with patch('src.tasks.document_processing.process_document_batch') as mock_doc_batch, \
         patch('src.tasks.analysis_tasks.analyze_document_batch') as mock_analysis_batch, \
         patch('src.tasks.scoring_tasks.calculate_scores_batch') as mock_scoring_batch:
...
def mock_llm_service():
    ...
    with patch('src.llm.langchain_service.LangChainLLMService') as mock_service:
        ...
        mock_instance.generate_analysis = AsyncMock(return_value={
            "summary": "Test analysis summary",
            "key_points": ["Point 1", "Point 2"],
            "sentiment": "neutral",
            "confidence": 0.85
        })
        mock_instance.generate_score_rationale = AsyncMock(return_value={
...
def validate_test_outputs():
    ...
    def _validate(state: WorkflowState):
        ...
        validations = {
            ...
            'no_critical_errors': len(state.errors) == 0 or all("critical" not in str(e).lower() for e in state.errors),

...
class ResultAggregator:
    ...
    def group_by_priority(self, scoring_results: List[ScoringResult]) -> List[PriorityQueue]:
        ...
        try:
            ...
            logger.info("Priority grouping completed", 
                       queue_count=len(priority_queues))
    ...
    def _empty_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_statistics": {"average": 0, "high_confidence_count": 0, "medium_confidence_count": 0, "low_confidence_count": 0},
            "priority_distribution": {},
            "dimension_analysis": {},
            "processing_statistics": {"total_time_ms": 0, "average_time_ms": 0}
        }
    
    def _calculate_priority_distribution(self, scoring_results: List[ScoringResult]) -> Dict[str, int]:
    ...
    def _calculate_processing_stats(self, scoring_results: List[ScoringResult]) -> Dict[str, float]:
        ...
        processing_times = [result.processing_time_ms for result in scoring_results if result.processing_time_ms > 0]
    ...
    def generate_insights(self, scoring_results: List[ScoringResult]) -> List[str]:
        ...
        if high_priority_count > 0:
            ...
            insights.append(f"{high_priority_count} high-priority documents identified ({percentage}% of total)")
        ...
        high_conf_pct = round(confidence_stats["high_confidence_count"] / metrics["total_documents"] * 100)
    ...
    def generate_recommendations(self, scoring_results: List[ScoringResult]) -> List[str]:
        ...
        if critical_count > 0:
            recommendations.append(f"Immediate review required for {critical_count} critical priority documents")
        
        # High priority recommendations
        high_count = priority_dist.get("high", 0)
        if high_count > 0:
            recommendations.append(f"Schedule review of {high_count} high priority documents within 1 week")
        
        # Medium priority recommendations
        medium_count = priority_dist.get("medium", 0)
        if medium_count > 5:
            recommendations.append(f"Batch review {medium_count} medium priority documents by priority sub-groups")
        ...
        if low_conf_count > 0:
            recommendations.append(f"Manual verification recommended for {low_conf_count} low-confidence results")
        
        # Processing recommendations
        if metrics["total_documents"] > 100:
            recommendations.append("Consider setting up automated monitoring for large document volumes")

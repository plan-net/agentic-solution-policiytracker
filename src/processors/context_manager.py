...
class ContextManager:
        ...
        for field, expected_type in required_fields.items():
            ...
            if not isinstance(context_data[field], expected_type):
                raise ConfigurationError(
                    "context_file", f"Field {field} must be a {expected_type.__name__}"
                )

            # Normalize to lowercase strings
            if expected_type == list:
                validated[field] = [
                    str(item).lower().strip() for item in context_data[field] if item
                ]
    ...
    def _validate_topic_patterns(self, topic_patterns: Dict[str, Any]) -> None:
        ...
        for topic_name, keywords in topic_patterns.items():
            if not isinstance(keywords, list):
                raise ConfigurationError(
                    "context_file", f"Keywords for topic '{topic_name}' must be a list"
                )
    ...
    def create_default_context(self, output_path: str) -> None:
        ...
        default_context = {
            "company_terms": ["your-company-name", "company-abbreviation"],
            "core_industries": ["e-commerce", "online retail", "digital marketplace"],
            "primary_markets": ["european union", "eu", "germany", "france"],
            "strategic_themes": [
            ...
            "secondary_markets": ["uk", "switzerland", "austria", "netherlands"],
            "exclusion_terms": ["unrelated-term-1", "unrelated-term-2"],
        }

...
class ContentProcessor:
    ...
    def __init__(self, use_azure: bool = None):
        ...
        self.use_azure = use_azure if use_azure is not None else getattr(settings, 'USE_AZURE_STORAGE', False)
        self.azure_client = AzureStorageClient() if self.use_azure else None
        self.cache_enabled = True  # Enable caching by default
    
    async def process_document(self, file_path: str, doc_id: str, job_id: Optional[str] = None) -> ProcessedContent:
        ...
        try:
            ...
            raw_text, metadata = await self.reader.read_document(file_path, use_azure=self.use_azure)
    ...
    def _extract_sections(self, text: str) -> List[Dict[str, str]]:
        ...
        for line in lines:
            ...
            # Title case headers
            elif (line.istitle() and len(line) < 100 and 
                  not any(p in line.lower() for p in ['.', ',', ';', ':', '(', ')'])):
                header_match = type('Match', (), {'group': lambda self, n: line})()
    ...
    def extract_metadata_keywords(self, text: str) -> List[str]:
        ...
        common_words = {'The', 'This', 'That', 'They', 'There', 'These', 'Those', 'When', 'Where', 'What', 'Who', 'Why', 'How'}
    ...
    async def _get_cached_content(self, file_path: str, doc_id: str) -> Optional[ProcessedContent]:
        ...
        try:
            ...
            if 'processing_timestamp' in cached_data:
                cached_data['processing_timestamp'] = datetime.fromisoformat(cached_data['processing_timestamp'])
            ...
            logger.warning("Failed to retrieve cached content", 
                         file_path=file_path, doc_id=doc_id, error=str(e))
            return None
    
    async def _cache_content(self, file_path: str, doc_id: str, content: ProcessedContent, job_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            if 'processing_timestamp' in content_dict:
                content_dict['processing_timestamp'] = content_dict['processing_timestamp'].isoformat()
            ...
            if success:
                logger.debug("Cached processed content", 
                           file_path=file_path, doc_id=doc_id, cache_key=cache_key)
            
            return success
            
        except Exception as e:
            logger.warning("Failed to cache processed content", 
                         file_path=file_path, doc_id=doc_id, error=str(e))
            return False
        ...
        try:
            ...
            for blob in blobs:
                try:
                    ...
                    logger.warning("Failed to delete cached blob", blob_name=blob.name, error=str(e))

...
from src.utils.validators import validate_file_extension, get_supported_extensions, validate_directory_exists
...
class BatchDocumentLoader:
    ...
    async def _discover_files_local(self, input_folder: str) -> List[str]:
        ...
        try:
            ...
            logger.info(f"Discovered {len(sorted_files)} local files", 
    ...
    async def _discover_files_azure(self, job_prefix: str, job_id: Optional[str] = None) -> List[str]:
        ...
        try:
            ...
            if job_id:
                ...
                blob_prefix = f"jobs/{job_prefix}/input/" if not job_prefix.startswith('jobs/') else job_prefix
            ...
            for blob in blobs:
                ...
                if blob_path.suffix.lower() in self.supported_extensions:
                    discovered_files.append(blob.name)
                    # Get blob size for sorting
                    file_info.append((blob.name, blob.size or 0))
            
            # Sort by size (smaller first for faster processing)
            file_info.sort(key=lambda x: x[1])
            sorted_files = [file_path for file_path, _ in file_info]
            
            logger.info(f"Discovered {len(sorted_files)} Azure Storage files", 
    ...
    async def _filter_by_size_azure(self, blob_names: List[str], max_size_mb: int) -> List[str]:
        ...
        for blob_name in blob_names:
            try:
                ...
                blob_properties = await self.azure_client.get_blob_properties('input-documents', blob_name)
                if blob_properties:
                    ...
                    if size_mb <= max_size_mb:
                        ...
                        logger.warning("Blob too large, skipping", 
                                     blob_name=blob_name, 
                                     size_mb=round(size_mb, 2),
                                     max_size_mb=max_size_mb)
                else:

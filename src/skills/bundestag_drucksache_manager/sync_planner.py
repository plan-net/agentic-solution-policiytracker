"""SyncPlanner - Convert Drucksache diffs into CRUD operations and batch them."""
import logging
from typing import Any, Dict, List

from .diff_analyzer import DrucksacheDiff
...
class DrucksacheSyncPlanner:
    """Plans CRUD operations based on detected Drucksache differences."""

    def __init__(self, batch_size: int = 50, max_concurrent: int = 100):
        """Initialize DrucksacheSyncPlanner.
        ...
        logger.info(
            f"DrucksacheSyncPlanner initialized (batch_size={batch_size}, max_concurrent={max_concurrent})"
        )

    def create_sync_plan(self, diffs: List[DrucksacheDiff]) -> SyncPlan:
    ...
    def _diff_to_operations(self, diff: DrucksacheDiff) -> List[Dict[str, Any]]:
        """Convert a DrucksacheDiff into CRUD operations.

        Args:
            diff: DrucksacheDiff object
        ...
        if diff.diff_type == "missing":
            # Drucksache doesn't exist in Neo4j - create it
            # Ensure 'drucksache_nummer' is present (required by ENTITY_ID_FIELDS)
            properties = {**diff.dip_data, "active": True}
            # The DIP client should already have drucksache_nummer, but double-check
            if "drucksache_nummer" not in properties and "id" in properties:
                properties["drucksache_nummer"] = properties["id"]
            ...
            operations.append(
                {
                    ...
                    "drucksache_id": diff.drucksache_id,
                }
            )

        elif diff.diff_type == "outdated":
            # Drucksache exists but has outdated fields - update it

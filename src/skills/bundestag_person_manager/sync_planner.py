...
class SyncPlanner:
    """Plans CRUD operations based on detected differences."""

    def __init__(self, batch_size: int = 50, max_concurrent: int = 100):
        """Initialize SyncPlanner.
    ...
    def _diff_to_operations(self, diff: PersonDiff) -> List[Dict[str, Any]]:
        ...
        if diff.diff_type == "missing":
            # Person doesn't exist in Neo4j - create it
            # Rename 'id' to 'person_id' for Neo4j compatibility
            properties = {**diff.dip_data, "active": True}
            if "id" in properties and "person_id" not in properties:
                properties["person_id"] = properties.pop("id")

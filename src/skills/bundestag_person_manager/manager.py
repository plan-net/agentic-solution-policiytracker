...
from .dip_client import BundestagDIPClient, MockBundestagDIPClient
from .sync_planner import SyncPlan, SyncPlanner
...
class BundestagPersonManager:
    """Manager for intelligent BundestagPerson data synchronization.
    ...
    def __init__(self, config: ManagerConfig = None, use_mock_dip: bool = False):
        ...
        if use_mock_dip:
            self.dip_client = MockBundestagDIPClient()
            logger.info("Using mock DIP client")
        else:
            self.dip_client = BundestagDIPClient(api_key=config.dip_api_key)
        ...
        logger.info("BundestagPersonManager initialized")

    async def sync_all_persons(
        self, limit: Optional[int] = None, dry_run: bool = False
    ) -> SyncResult:
        """Synchronize all persons from Bundestag DIP API to Neo4j.

        This is the main method for full data synchronization.

        Args:
            limit: Maximum number of persons to sync (None = all)
    ...
    async def sync_specific_persons(
        ...
        Args:
            person_ids: List of person IDs to sync
            dry_run: If True, analyze differences but don't execute operations
        ...
        try:
            ...
            diffs = await self.diff_analyzer.analyze_specific_persons(person_ids)
            diff_summary = self.diff_analyzer.generate_summary(diffs)

            if not diffs:
                logger.info("No differences found for specified persons")
                return SyncResult(
                    success=True,
                    operations_attempted=0,
                    operations_succeeded=0,
    ...
    async def check_sync_status(self) -> dict[str, Any]:
        ...
        diffs = await self.diff_analyzer.analyze_all_persons(limit=100)
        ...
        return {
            ...
            "missing_persons": summary["missing_count"],
            "outdated_persons": summary["outdated_count"],
    ...
    def close(self):
        """Clean up resources."""
        logger.info("Closing BundestagPersonManager...")
        ...
        logger.info("BundestagPersonManager closed")

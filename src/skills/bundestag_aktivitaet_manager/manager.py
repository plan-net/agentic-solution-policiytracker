"""BundestagAktivitaet Manager - Main orchestration for intelligent Neo4j sync."""
import logging
...
from .diff_analyzer import AktivitaetDiffAnalyzer
from .dip_client import BundestagAktivitaetDIPClient, MockBundestagAktivitaetDIPClient
from .sync_planner import AktivitaetSyncPlanner, SyncPlan
...
class BundestagAktivitaetManager:
    """Manager for intelligent BundestagAktivitaet data synchronization.
    ...
    def __init__(self, config: ManagerConfig = None, use_mock_dip: bool = False):
        """Initialize BundestagAktivitaet Manager.
        ...
        if use_mock_dip:
            self.dip_client = MockBundestagAktivitaetDIPClient()
            logger.info("Using mock DIP client for Aktivitaet")
        else:
            self.dip_client = BundestagAktivitaetDIPClient(api_key=config.dip_api_key)
            logger.info("Using real DIP client for Aktivitaet")

        # Initialize components
        self.diff_analyzer = AktivitaetDiffAnalyzer(
            self.neo4j_driver, config.neo4j_database, self.dip_client
        )
        self.sync_planner = AktivitaetSyncPlanner(
        ...
        logger.info("BundestagAktivitaetManager initialized")

    async def sync_all_aktivitaeten(
        self, limit: Optional[int] = None, dry_run: bool = False
    ) -> SyncResult:
        """Synchronize all Aktivitaetn from Bundestag DIP API to Neo4j.

        This is the main method for full data synchronization.

        Args:
            limit: Maximum number of Aktivitaetn to sync (None = all)
            dry_run: If True, analyze differences but don't execute operations

        Returns:
            SyncResult with execution details
        """
        logger.info(f"Starting full Aktivitaet sync (limit={limit}, dry_run={dry_run})")
        start_time = datetime.now()

        try:
            # Step 1: Analyze differences
            logger.info("Step 1: Analyzing differences...")
            diffs = await self.diff_analyzer.analyze_all_aktivitaeten(limit=limit)
            ...
            logger.error(f"Error during Aktivitaet sync: {e}", exc_info=True)
            total_time = (datetime.now() - start_time).total_seconds()
            return SyncResult(
                success=False,
                operations_attempted=0,
    ...
    async def sync_specific_aktivitaeten(
        self, aktivitaet_ids: list[str], dry_run: bool = False
    ) -> SyncResult:
        """Synchronize specific Aktivitaetn.

        Args:
            aktivitaet_ids: List of Aktivitaet IDs to sync
            dry_run: If True, analyze differences but don't execute operations
        ...
        """
        logger.info(
            f"Starting sync for {len(aktivitaet_ids)} specific Aktivitaetn (dry_run={dry_run})"
        )
        start_time = datetime.now()

        try:
            # Analyze specific Aktivitaetn
            diffs = await self.diff_analyzer.analyze_specific_aktivitaeten(aktivitaet_ids)
            diff_summary = self.diff_analyzer.generate_summary(diffs)

            if not diffs:
                logger.info("No differences found for specified Aktivitaetn")
                return SyncResult(
                    success=True,
                    operations_attempted=0,
                    operations_succeeded=0,
    ...
    async def check_sync_status(self) -> dict[str, Any]:
        ...
        """
        logger.info("Checking Aktivitaet sync status...")

        diffs = await self.diff_analyzer.analyze_all_aktivitaeten(limit=100)
        ...
        return {
            ...
            "missing_aktivitaeten": summary["missing_count"],
            "outdated_aktivitaeten": summary["outdated_count"],
    ...
    def close(self):
        """Clean up resources."""
        logger.info("Closing BundestagAktivitaetManager...")
        ...
        logger.info("BundestagAktivitaetManager closed")

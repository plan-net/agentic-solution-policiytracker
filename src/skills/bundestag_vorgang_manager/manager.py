"""BundestagVorgang Manager - Main orchestration for intelligent Neo4j sync."""
import asyncio
import logging
...
from .diff_analyzer import VorgangDiffAnalyzer
from .dip_client import BundestagVorgangDIPClient, MockBundestagVorgangDIPClient
from .sync_planner import SyncPlan, VorgangSyncPlanner
...
class BundestagVorgangManager:
    """Manager for intelligent BundestagVorgang data synchronization.
    ...
    def __init__(
        ...
        """Initialize BundestagVorgang Manager.
        ...
        if use_mock_dip:
            self.dip_client = MockBundestagVorgangDIPClient()
            logger.info("Using mock DIP client for Vorgang")
        else:
            self.dip_client = BundestagVorgangDIPClient(api_key=config.dip_api_key)
            logger.info("Using real DIP client for Vorgang")

        # Initialize components
        self.diff_analyzer = VorgangDiffAnalyzer(
            self.neo4j_driver, config.neo4j_database, self.dip_client
        )
        self.sync_planner = VorgangSyncPlanner(
    ...
    async def sync_all_vorgaenge(
        ...
        """
        logger.info(
            f"Starting full Vorgang sync (limit={limit}, dry_run={dry_run})"
        )
        ...
        try:
            ...
            logger.error(f"Error during Vorgang sync: {e}", exc_info=True)
            total_time = (datetime.now() - start_time).total_seconds()
            return SyncResult(
                success=False,
                operations_attempted=0,
    ...
    async def sync_specific_vorgaenge(
        self, vorgang_ids: List[str], dry_run: bool = False
    ) -> SyncResult:
        """Synchronize specific Vorgänge.

        Args:
            vorgang_ids: List of Vorgang IDs to sync
            dry_run: If True, analyze differences but don't execute operations
        ...
        """
        logger.info(
            f"Starting sync for {len(vorgang_ids)} specific Vorgänge (dry_run={dry_run})"
        )
        start_time = datetime.now()

        try:
            # Analyze specific Vorgänge
            diffs = await self.diff_analyzer.analyze_specific_vorgaenge(vorgang_ids)
            diff_summary = self.diff_analyzer.generate_summary(diffs)

            if not diffs:
                logger.info("No differences found for specified Vorgänge")
                return SyncResult(
                    success=True,
                    operations_attempted=0,
                    operations_succeeded=0,
    ...
    async def check_sync_status(self) -> Dict[str, Any]:
        ...
        """
        logger.info("Checking Vorgang sync status...")

        diffs = await self.diff_analyzer.analyze_all_vorgaenge(limit=100)
        ...
        return {
            ...
            "missing_vorgaenge": summary["missing_count"],
            "outdated_vorgaenge": summary["outdated_count"],
    ...
    def close(self):
        """Clean up resources."""
        logger.info("Closing BundestagVorgangManager...")
        ...
        logger.info("BundestagVorgangManager closed")

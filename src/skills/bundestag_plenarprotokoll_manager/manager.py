"""BundestagPlenarprotokoll Manager - Main orchestration for intelligent Neo4j sync."""
import logging
...
from .diff_analyzer import PlenarprotokollDiffAnalyzer
from .dip_client import BundestagPlenarprotokollDIPClient, MockBundestagPlenarprotokollDIPClient
from .sync_planner import PlenarprotokollSyncPlanner, SyncPlan
...
class BundestagPlenarprotokollManager:
    """Manager for intelligent BundestagPlenarprotokoll data synchronization.
    ...
    def __init__(self, config: ManagerConfig = None, use_mock_dip: bool = False):
        """Initialize BundestagPlenarprotokoll Manager.
        ...
        if use_mock_dip:
            self.dip_client = MockBundestagPlenarprotokollDIPClient()
            logger.info("Using mock DIP client for Plenarprotokoll")
        else:
            self.dip_client = BundestagPlenarprotokollDIPClient(api_key=config.dip_api_key)
            logger.info("Using real DIP client for Plenarprotokoll")

        # Initialize components
        self.diff_analyzer = PlenarprotokollDiffAnalyzer(
            self.neo4j_driver, config.neo4j_database, self.dip_client
        )
        self.sync_planner = PlenarprotokollSyncPlanner(
        ...
        logger.info("BundestagPlenarprotokollManager initialized")

    async def sync_all_plenarprotokolle(
        self, limit: Optional[int] = None, dry_run: bool = False
    ) -> SyncResult:
        """Synchronize all Plenarprotokolln from Bundestag DIP API to Neo4j.

        This is the main method for full data synchronization.

        Args:
            limit: Maximum number of Plenarprotokolln to sync (None = all)
            dry_run: If True, analyze differences but don't execute operations

        Returns:
            SyncResult with execution details
        """
        logger.info(f"Starting full Plenarprotokoll sync (limit={limit}, dry_run={dry_run})")
        start_time = datetime.now()

        try:
            # Step 1: Analyze differences
            logger.info("Step 1: Analyzing differences...")
            diffs = await self.diff_analyzer.analyze_all_plenarprotokolle(limit=limit)
            ...
            logger.error(f"Error during Plenarprotokoll sync: {e}", exc_info=True)
            total_time = (datetime.now() - start_time).total_seconds()
            return SyncResult(
                success=False,
                operations_attempted=0,
    ...
    async def sync_specific_plenarprotokolle(
        self, plenarprotokoll_ids: list[str], dry_run: bool = False
    ) -> SyncResult:
        """Synchronize specific Plenarprotokolln.

        Args:
            plenarprotokoll_ids: List of Plenarprotokoll IDs to sync
            dry_run: If True, analyze differences but don't execute operations
        ...
        """
        logger.info(
            f"Starting sync for {len(plenarprotokoll_ids)} specific Plenarprotokolln (dry_run={dry_run})"
        )
        start_time = datetime.now()

        try:
            # Analyze specific Plenarprotokolln
            diffs = await self.diff_analyzer.analyze_specific_plenarprotokolle(plenarprotokoll_ids)
            diff_summary = self.diff_analyzer.generate_summary(diffs)

            if not diffs:
                logger.info("No differences found for specified Plenarprotokolln")
                return SyncResult(
                    success=True,
                    operations_attempted=0,
                    operations_succeeded=0,
    ...
    async def check_sync_status(self) -> dict[str, Any]:
        ...
        """
        logger.info("Checking Plenarprotokoll sync status...")

        diffs = await self.diff_analyzer.analyze_all_plenarprotokolle(limit=100)
        ...
        return {
            ...
            "missing_plenarprotokolle": summary["missing_count"],
            "outdated_plenarprotokolle": summary["outdated_count"],
    ...
    def close(self):
        """Clean up resources."""
        logger.info("Closing BundestagPlenarprotokollManager...")
        ...
        logger.info("BundestagPlenarprotokollManager closed")

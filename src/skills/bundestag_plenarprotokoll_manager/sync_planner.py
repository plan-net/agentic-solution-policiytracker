...
class PlenarprotokollSyncPlanner:
    """Plans CRUD operations based on detected Plenarprotokoll differences."""

    def __init__(self, batch_size: int = 50, max_concurrent: int = 100):
        """Initialize PlenarprotokollSyncPlanner.
    ...
    def _diff_to_operations(self, diff: PlenarprotokollDiff) -> List[Dict[str, Any]]:
        ...
        Args:
            diff: PlenarprotokollDiff object
        ...
        if diff.diff_type == "missing":
            # Plenarprotokoll doesn't exist in Neo4j - create it
            # Ensure 'plenarprotokoll_nummer' is present (required by ENTITY_ID_FIELDS)
            properties = {**diff.dip_data, "active": True}
            # The DIP client should already have plenarprotokoll_nummer, but double-check
            if "plenarprotokoll_nummer" not in properties and "id" in properties:
                properties["plenarprotokoll_nummer"] = properties["id"]
            ...
            operations.append(
                {
                    ...
                    "plenarprotokoll_id": diff.plenarprotokoll_id,
                }
            )
            ...
            operations.append(
                {
                    ...
                    "entity_type": "Plenarprotokoll",
                    "node_id": diff.plenarprotokoll_id,

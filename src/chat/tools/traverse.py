...
class TraverseFromEntityInput(BaseModel):
    ...
    relationship_types: Optional[List[str]] = Field(default=None, description="Types of relationships to follow (e.g., ['AFFECTS', 'GOVERNS'])")
...
class GetNeighborsInput(BaseModel):
    ...
    neighbor_types: Optional[List[str]] = Field(default=None, description="Types of entities to focus on as neighbors")
...
class ImpactAnalysisInput(BaseModel):
    ...
    impact_types: Optional[List[str]] = Field(default=None, description="Types of impact to focus on (e.g., ['regulatory', 'financial'])")
...
class TraverseFromEntityTool(BaseTool):
    ...
    def _run(self, entity_name: str, relationship_types: Optional[List[str]] = None, max_depth: int = 2, max_results: int = 15, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, entity_name: str, relationship_types: Optional[List[str]] = None, max_depth: int = 2, max_results: int = 15, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            for depth in sorted(depth_groups.keys()):
                ...
                for rel in depth_groups[depth][:5]:  # Top 5 per level
                    response += f"- **{rel['target_entity']}** [{rel['type']}]: {rel['fact'][:100]}...\n"
            ...
            logger.info(f"Traversal found {len(relationships_found)} relationships across {len(depth_groups)} levels")
            return response
...
class FindPathsTool(BaseTool):
    ...
    def _run(self, source_entity: str, target_entity: str, max_path_length: int = 4, max_paths: int = 5, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, source_entity: str, target_entity: str, max_path_length: int = 4, max_paths: int = 5, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            for result in results:
                ...
                if (source_entity.lower() in fact_lower and 
                    target_entity.lower() in fact_lower):
            ...
            logger.info(f"Found {len(direct_connections)} direct and {len(indirect_connections)} indirect connections")
            return response
...
class GetNeighborsTool(BaseTool):
    ...
    def _run(self, entity_name: str, max_depth: int = 1, neighbor_types: Optional[List[str]] = None, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, entity_name: str, max_depth: int = 1, neighbor_types: Optional[List[str]] = None, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            for result in results:
                ...
                if entity_name.lower() in fact.lower():
                    ...
                    for neighbor in potential_neighbors:
                        if (neighbor.lower() != entity_name.lower() and 
                            len(neighbor) > 2):
                            
                            # Filter by neighbor types if specified
                            if neighbor_types:
                                neighbor_matches = any(ntype.lower() in fact.lower() 
            ...
            if neighbors:
                ...
                response += f"### Relationship Types Found:\n"
                for rel_type in sorted(relationship_types):
                    count = sum(1 for conns in neighbors.values() 
                              for conn in conns if conn['relationship'] == rel_type)
                    response += f"- {rel_type}: {count} connections\n"
                
            else:
                response += f"No clear neighboring entities found for {entity_name}."
            
            logger.info(f"Found {len(neighbors)} neighbors with {len(relationship_types)} relationship types")
            return response
...
class ImpactAnalysisTool(BaseTool):
    ...
    def _run(self, entity_name: str, impact_types: Optional[List[str]] = None, max_hops: int = 3, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, entity_name: str, impact_types: Optional[List[str]] = None, max_hops: int = 3, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            if direct_impacts:
                response += f"### What {entity_name} Impacts ({len(direct_impacts)} relationships):\n"
            ...
            if indirect_impacts:
                response += f"### What Impacts {entity_name} ({len(indirect_impacts)} relationships):\n"
            ...
            # Mutual impacts
            if mutual_impacts:
                response += f"### Mutual/Bidirectional Impacts ({len(mutual_impacts)} relationships):\n"

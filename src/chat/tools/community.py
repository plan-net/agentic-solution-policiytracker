...
class GetCommunitiesInput(BaseModel):
    ...
    topic_focus: Optional[str] = Field(default=None, description="Topic to focus communities on (e.g., 'AI regulation', 'privacy')")
    max_communities: int = Field(default=5, description="Maximum number of communities to return")
    min_community_size: int = Field(default=3, description="Minimum number of entities per community")
...
class CommunityMembersInput(BaseModel):
    ...
    max_members: int = Field(default=10, description="Maximum number of community members to return")
    member_types: Optional[List[str]] = Field(default=None, description="Types of entities to focus on (e.g., ['Policy', 'Company'])")
...
class RelatedCommunitiesInput(BaseModel):
    ...
    max_communities: int = Field(default=5, description="Maximum number of related communities to return")
    relationship_types: Optional[List[str]] = Field(default=None, description="Types of relationships to consider for community detection")


class PolicyClustersInput(BaseModel):
    """Input schema for policy clusters tool."""
    policy_area: Optional[str] = Field(default=None, description="Policy area to focus on (e.g., 'digital', 'environmental')")
    cluster_method: str = Field(default="thematic", description="Clustering method: 'thematic', 'jurisdictional', or 'temporal'")
...
class OrganizationNetworkInput(BaseModel):
    ...
    organization_type: Optional[str] = Field(default=None, description="Type of organizations to focus on (e.g., 'regulatory', 'tech')")
    network_depth: int = Field(default=2, description="Depth of network connections to explore")
    min_connections: int = Field(default=2, description="Minimum connections to include organization in network")
...
class GetCommunitiesTool(BaseTool):
    ...
    def _run(self, topic_focus: Optional[str] = None, max_communities: int = 5, min_community_size: int = 3, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, topic_focus: Optional[str] = None, max_communities: int = 5, min_community_size: int = 3, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            if topic_focus:
                ...
                search_query = "communities groups clusters networks related connected policy organization"
            ...
            response = f"## Community Detection Analysis\n\n"
            if topic_focus:
            ...
            if communities:
                ...
                for i, community in enumerate(communities, 1):
                    ...
                    if community['connections']:
                        response += f"**Main Connections**: {', '.join(community['connections'][:3])}\n"
                    response += "\n"
                ...
                response += f"No clear communities detected for the given criteria.\n\n"
    ...
    def _detect_communities(self, results: List[Any], topic_focus: Optional[str], min_size: int) -> List[Dict[str, Any]]:
        ...
        for entity, cooccurrences in entity_cooccurrence.items():
            ...
            for related_entity, strength in sorted_cooccurrences[:10]:  # Top 10 related
                if related_entity not in processed_entities and strength >= 2:  # Minimum co-occurrence
            ...
            if len(community_members) >= min_size:
                ...
                cohesion = self._calculate_community_cohesion(community_members, entity_cooccurrence)
                
                # Determine community theme
                theme = self._determine_community_theme(community_members, entity_contexts, topic_focus)
    ...
    def _extract_entities_from_content(self, content: str) -> List[str]:
        ...
        for entity in entity_patterns:
            ...
            if len(entity) > 2 and len(entity) < 50:
                ...
                if not any(word in entity.lower() for word in ['the', 'and', 'or', 'but', 'this', 'that']):
    ...
    def _calculate_community_cohesion(self, members: List[str], cooccurrence_matrix: Dict[str, Dict[str, int]]) -> float:
    ...
    def _determine_community_theme(self, members: List[str], contexts: Dict[str, List[str]], topic_focus: Optional[str]) -> str:
        ...
        if topic_focus and any(focus_word in combined_text for focus_word in topic_focus.lower().split()):
...
class CommunityMembersTool(BaseTool):
    ...
    def _run(self, community_topic: str, max_members: int = 10, member_types: Optional[List[str]] = None, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, community_topic: str, max_members: int = 10, member_types: Optional[List[str]] = None, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            search_query = f"{community_topic} members organizations companies policies entities related"
            if member_types:
    ...
    def _extract_community_members(self, results: List[Any], community_topic: str, member_types: Optional[List[str]]) -> List[Dict[str, Any]]:
                ...
                for entity in entities:
                    if entity not in processed_names:
                        ...
                        if relevance > 0.3:  # Minimum relevance threshold
                            ...
                            if member_types and member_type not in [mt.lower() for mt in member_types]:
                                continue
    ...
    def _extract_entities_from_content(self, content: str) -> List[str]:
        ...
        for entity in entity_patterns:
            ...
            if len(entity) > 2 and len(entity) < 50:
                # Remove common non-entity words
                if not any(word in entity.lower() for word in ['the', 'and', 'or', 'but', 'this', 'that', 'which', 'what']):
                    entities.append(entity)
    ...
    def _extract_member_connections(self, entity: str, content: str) -> List[str]:
        ...
        for other_entity in entities:
            ...
            if (other_entity != entity and 
                len(other_entity) > 2 and 
                len(other_entity) < 50 and
                not any(word in other_entity.lower() for word in ['the', 'and', 'or', 'but'])):
...
class PolicyClustersTool(BaseTool):
    ...
    def _run(self, policy_area: Optional[str] = None, cluster_method: str = "thematic", max_clusters: int = 5, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    
    async def _arun(self, policy_area: Optional[str] = None, cluster_method: str = "thematic", max_clusters: int = 5, run_manager: Optional[CallbackManagerForToolRun] = None) -> str:
        ...
        try:
            ...
            response = f"## Policy Clusters Analysis\n\n"
            if policy_area:
            ...
            if policy_clusters:
                ...
                for i, cluster in enumerate(policy_clusters, 1):
                    ...
                    if cluster['relationships']:
                        response += f"**Common Relationships**: {', '.join(cluster['relationships'][:3])}\n"
                    response += "\n"
                ...
                response += f"No clear policy clusters found.\n\n"
                response += "**Suggestions:**\n"
    ...
    def _cluster_policies(self, results: List[Any], cluster_method: str, policy_area: Optional[str]) -> List[Dict[str, Any]]:
    ...
    def _extract_policies_from_content(self, content: str, policy_area: Optional[str]) -> List[Dict[str, Any]]:
    ...
    def _extract_policy_theme(self, content: str, policy_name: str, policy_area: Optional[str]) -> str:
        ...
        for theme, keywords in themes.items():
            score = sum(content_lower.count(keyword) + policy_lower.count(keyword) for keyword in keywords)
            if score > 0:
                theme_scores[theme] = score
    ...
    def _calculate_policy_cohesion(self, policies: List[Dict[str, Any]], cluster_type: str) -> float:

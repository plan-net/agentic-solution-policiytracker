...
class EntityHistoryInput(BaseModel):
    ...
    days_back: int = Field(
        default=365, description="Number of days back to search for entity history"
    )
    event_types: Optional[list[str]] = Field(
        default=None, description="Types of events to focus on (e.g., 'regulatory', 'enforcement')"
    )
...
class ConcurrentEventsInput(BaseModel):
    ...
    window_days: int = Field(
        default=30, description="Number of days before and after reference date to search"
    )
    event_context: Optional[str] = Field(
        default=None, description="Context or theme to focus concurrent events on"
    )
...
class TimelineComparisonInput(BaseModel):
    ...
    comparison_focus: Optional[str] = Field(
        default=None, description="Specific aspect to focus comparison on"
    )
...
class PolicyEvolutionInput(BaseModel):
    ...
    evolution_period: int = Field(
        default=730, description="Period in days to track evolution (default: 2 years)"
    )
    evolution_aspects: Optional[list[str]] = Field(
        default=None,
        description="Specific aspects to track (e.g., 'amendments', 'enforcement', 'compliance')",
    )
...
class DateRangeSearchTool(BaseTool):
    ...
    async def _arun(
        ...
        try:
            logger.info(
                f"Searching date range {start_date.date()} to {end_date.date()} for: {query}"
            )
            ...
            search_results = await self.client._search(
                query=temporal_query, config=EDGE_HYBRID_SEARCH_EPISODE_MENTIONS
            )
            ...
            if temporal_events:
                ...
                for i, event in enumerate(temporal_events[:max_results], 1):
                    ...
                    if event["temporal_score"] > 0.7:
                        response += (
                            f"   *High temporal relevance ({event['temporal_score']:.2f})*\n"
                        )
                    response += "\n"
    ...
    def _calculate_temporal_relevance(
        self, content: str, start_date: datetime, end_date: datetime
    ) -> float:
...
class EntityHistoryTool(BaseTool):
    ...
    async def _arun(
        ...
        try:
            ...
            search_results = await self.client._search(
                query=search_query, config=EDGE_HYBRID_SEARCH_EPISODE_MENTIONS
            )
            ...
            historical_events.sort(
                key=lambda x: (x["importance"], len(x["temporal_indicators"])), reverse=True
            )
...
class ConcurrentEventsTool(BaseTool):
    ...
    async def _arun(
        ...
        try:
            logger.info(
                f"Finding concurrent events around {reference_date.date()} (±{window_days} days)"
            )
            ...
            for result in results:
                ...
                if content:
                    ...
                    temporal_relevance = self._check_temporal_proximity(
                        content, reference_date, window_days
                    )
            ...
            response = "## Concurrent Events Analysis\n\n"
            response += f"**Reference Date**: {reference_date.date()}\n"
            response += (
                f"**Time Window**: ±{window_days} days ({start_date.date()} to {end_date.date()})\n"
            )
            if event_context:
            ...
            if concurrent_events:
                ...
                high_relevance = len(
                    [e for e in concurrent_events if e["temporal_relevance"] > 0.7]
                )
                ...
                response += (
                    f"No significant concurrent events found around {reference_date.date()}.\n\n"
                )
                response += "**Suggestions:**\n"
    ...
    def _check_temporal_proximity(
        self, content: str, reference_date: datetime, window_days: int
    ) -> float:
...
class PolicyEvolutionTool(BaseTool):
    ...
    def _run(
        ...
        evolution_aspects: Optional[list[str]] = None,
        run_manager: Optional[CallbackManagerForToolRun] = None,
    ) -> str:
        """Execute synchronously - placeholder."""
        return "Sync not implemented - use async version"
    ...
    async def _arun(
        ...
        try:
            ...
            evolution_events.sort(
                key=lambda x: (x["impact_level"], x["phase"] == "implementation"), reverse=True
            )
            ...
            if evolution_events:
                ...
                ordered_phases = {
                    phase: phases.get(phase, []) for phase in phase_order if phase in phases
                }
                ...
                for phase, events in ordered_phases.items():
                    ...
                    for i, event in enumerate(events[:4], 1):  # Top 4 per phase
                        ...
                        if event["stakeholders"]:
                            response += (
                                f"   *Stakeholders: {', '.join(event['stakeholders'][:3])}*\n"
                            )

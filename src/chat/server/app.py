...
class ChatServer:
    ...
    async def debug_test_stream(self, query: str = "What is GDPR?"):
        ...
        try:
            ...
            async for chunk in streaming_orchestrator.process_query_with_streaming(
                query=query, session_id=session_id, user_id="debug_user"
            ):
                chunk_info = {
                    ...
                    "data_keys": list(chunk.get("data", {}).keys())
                    if isinstance(chunk.get("data"), dict)
                    else "N/A",
                    "data_preview": str(chunk.get("data", ""))[:200] + "..."
                    if chunk.get("data")
                    else "None",
                }
    ...
    async def _stream_chat_response(
        ...
        try:
            ...
            async for chunk in streaming_orchestrator.graph.astream(
                initial_state, config=thread_config, stream_mode=["updates", "custom"]
            ):
            ...
            if final_response:
                ...
                if errors:
                    ...
                    yield self._create_chunk(
                        chat_id, created, model, f"I encountered an issue: {error_msg}"
                    )
                else:
                    yield self._create_chunk(
                        chat_id,
    ...
    def _create_chunk(self, chat_id: str, created: int, model: str, content: str) -> str:
        ...
        chunk = StreamResponse(
            ...
            created=created,
            model=model,
            choices=[StreamChoice(index=0, delta={"content": content}, finish_reason=None)],
        )
    ...
    async def _stream_with_enhanced_agent(
        self, model: str, user_message: str
    ) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            async for thinking_chunk in agent.stream_query(user_message):
                ...
                chunk = StreamResponse(
                    ...
                    model=model,
                    choices=[
                        StreamChoice(index=0, delta={"content": thinking_chunk}, finish_reason=None)
                    ],
                )

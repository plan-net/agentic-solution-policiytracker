...
class ResponseQualityAssessment:
    ...
    async def assess_response_quality(
        ...
        try:
            ...
            dimension_scores[QualityDimension.COHERENCE] = await self._assess_coherence(response)
    ...
    async def _assess_completeness(
        ...
        if secondary_entities:
            entity_coverage = sum(
                1 for entity in secondary_entities if entity.lower() in response.lower()
    ...
    async def _assess_accuracy(
        self, response: str, tool_results: list[dict[str, Any]], response_metadata: dict[str, Any]
    ) -> float:
    ...
    async def _assess_relevance(
        self, response: str, query: str, query_analysis: dict[str, Any]
    ) -> float:
    ...
    async def _assess_timeliness(
        self, response: str, tool_results: list[dict[str, Any]], execution_metadata: dict[str, Any]
    ) -> float:
        ...
        for dimension, score in dimension_scores.items():
            ...
            if score < threshold:
                if dimension == QualityDimension.COMPLETENESS:
                    ...
                    suggestions.append("Include more current information and recent developments")
                    ...
                    suggestions.append("Include more official sources and regulatory citations")
    ...
    async def _generate_quality_indicators(
        ...
        return {
            ...
            "contains_citations": any(
                keyword in response.lower() for keyword in ["source", "according to", "based on"]
            ),
    ...
    def _count_technical_terms(self, response: str) -> int:
        ...
        return sum(1 for term in technical_terms if term in response.lower())
...
class QualityOptimizer:
    ...
    async def suggest_optimizations(
        ...
        low_scoring_dimensions = [
            dim
            for dim, score in quality_metrics.dimension_scores.items()
            if score < self.assessor.quality_thresholds[dim]
        ]

        # Sort by importance (weight)
        low_scoring_dimensions.sort(
            key=lambda dim: self.assessor.quality_weights[dim], reverse=True
        )

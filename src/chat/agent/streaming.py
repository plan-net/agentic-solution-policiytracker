...
class StreamingThinkingGenerator:
    ...
    async def generate_query_understanding_thinking(
        self, agent_name: str, query: str, analysis_results: dict[str, Any]
    ) -> None:
        ...
        if "intent" in analysis_results:
            await self.streaming_manager.emit_thinking(
                agent_name, templates["intent_analysis"].format(intent=analysis_results["intent"])
            )
        ...
        if "entities" in analysis_results and analysis_results["entities"]:
            ...
            await self.streaming_manager.emit_thinking(
                agent_name, templates["entity_extraction"].format(entities=entity_list)
            )
        ...
        if "complexity" in analysis_results:
            await self.streaming_manager.emit_thinking(
                agent_name,
                templates["complexity_assessment"].format(
    ...
    async def generate_tool_planning_thinking(self, agent_name: str, plan: dict[str, Any]) -> None:
        ...
        if "tool_sequence" in plan and len(plan["tool_sequence"]) > 0:
            ...
            await self.streaming_manager.emit_thinking(
                ...
                templates["tool_sequencing"].format(
                    first_tool=first_tool, subsequent_tools=subsequent_tools
                ),
            )
            await asyncio.sleep(0.6)
        ...
        if execution_status == "start":
            await self.streaming_manager.emit_thinking(
                ...
                templates["tool_start"].format(
                    tool_name=tool_name, purpose=self._get_tool_purpose(tool_name)
                ),
            )
            ...
            await self.streaming_manager.emit_thinking(
                ...
                templates["tool_complete"].format(
                    tool_name=tool_name,
                    insight_summary=results_summary.get(
                        "insight_summary", "valuable insights gathered"
                    ),
                ),
            )

    async def generate_synthesis_thinking(
        self, agent_name: str, synthesis_stage: str, stage_data: dict[str, Any] = None
    ) -> None:
...
async def aggregate_stream_events(events: list[dict[str, Any]]) -> dict[str, Any]:

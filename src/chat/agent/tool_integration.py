...
from ..tools.entity import (
    EntityDetailsTool, EntityRelationshipsTool, EntityTimelineTool, SimilarEntitesTool
)
from ..tools.traverse import (
    TraverseFromEntityTool, FindPathsTool, ImpactAnalysisTool
)
from ..tools.temporal import (
    DateRangeSearchTool, ConcurrentEventsTool, PolicyEvolutionTool
)
from ..tools.community import (
    GetCommunitiesTool, CommunityMembersTool, PolicyClustersTool
)
        ...
        if intent == "information_seeking":
            if strategy_type == "focused":
                recommendations.extend([
                    {
                        "tool_name": "search",
                        "parameters": {"query": " ".join(entities[:2]), "limit": 5, "search_type": "comprehensive"},
                ...
                recommendations.extend([
                    {
                        "tool_name": "search",
                        "parameters": {"query": " ".join(entities), "limit": 10, "search_type": "comprehensive"},
                ...
                recommendations.extend([
                    {
                        ...
                        "parameters": {"start_entity": entities[0], "end_entity": entities[1], "max_hops": 3},
                    ...
                    {
                        "tool_name": "search",
                        "parameters": {"query": f"{entities[0]} {entities[1]}", "search_type": "relationship_focused"},

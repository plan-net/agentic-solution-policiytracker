...
class ApifyNewsCollector:
    ...
    async def collect_news(
        ...
        try:
            ...
            for item in self.client.dataset(run["defaultDatasetId"]).iterate_items():
                ...
                raw_date = item.get("publishedAt", item.get("date"))
                if raw_date:
                    try:
                        # Parse raw date to check against start_date
                        parsed_date = None
                        for fmt in [
                            "%Y-%m-%dT%H:%M:%S.%fZ",
                            "%Y-%m-%dT%H:%M:%SZ",
    ...
    def _normalize_article(self, raw_article: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "content": raw_article.get(
                "text", raw_article.get("content", "")
            ),  # May be empty if fetchArticleDetails=false
            # Temporal fields (important for Graphiti)
            "published_date": self._parse_date(
                raw_article.get("publishedAt", raw_article.get("date"))
            ),

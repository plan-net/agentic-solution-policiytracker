...
def load_client_config(**context):
    ...
    print(f"Collection mode: {'INITIALIZATION' if is_initialization else 'DAILY'} ({days_back} days back)")
...
def collect_news_data(**context):
    ...
    print(f"Collected {len(articles)} articles for {company_name} using {collector_type} ({mode}: {days_back} days)")
    return len(articles)
...
def mark_initialization_complete(**context):
    ...
    if was_initialization:
        ...
        print(f"✅ Marked {collector_used} as initialized with {saved_count} articles from {days_back_used} days")
    else:

...
def check_policy_initialization(**context) -> dict[str, Any]:
    ...
    try:
        ...
        is_initialized = tracker.is_initialized("policy_landscape")
        collection_days = tracker.get_collection_days("policy_landscape")
        ...
        context["task_instance"].xcom_push(key="initialization_info", value=initialization_info)
...
def collect_policy_documents(**context) -> dict[str, Any]:
    ...
    try:
        ...
        initialization_info = context["task_instance"].xcom_pull(
            task_ids="check_policy_initialization", key="initialization_info"
        )
        ...
        context["task_instance"].xcom_push(key="collection_result", value=collection_result)
...
def _get_recommended_actions(collection_result: dict[str, Any]) -> list:
    ...
    if queries_processed > 0:
        ...
        if failure_rate > 0.2:  # >20% failure rate
            actions.append(
                f"High query failure rate ({failure_rate:.1%}). Check API limits and query validity."
            )
    ...
    if documents_saved < 10:
        actions.append(
            "Low document yield. Consider expanding search queries or increasing time range."
        )
    ...
    if processing_time > 300:  # >5 minutes
        actions.append(
            "Slow processing detected. Consider reducing concurrent queries or batch sizes."
        )

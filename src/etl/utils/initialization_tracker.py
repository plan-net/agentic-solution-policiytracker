...
class ETLInitializationTracker:
    ...
    def mark_initialized(
        self, collector_type: str, initialization_days: int, articles_collected: int
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"Marked {collector_type} as initialized with {articles_collected} articles from {initialization_days} days"
            )
    ...
    def get_collection_days(self, collector_type: str) -> int:
        ...
        if self.is_initialized(collector_type):
            logger.info(
                f"Collector {collector_type} already initialized, using daily collection: {daily_days} days"
            )
            return daily_days
        else:
            logger.info(
                f"Collector {collector_type} not initialized, using initialization period: {initialization_days} days"
            )
    ...
    def get_tracker_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "tracker_file": str(self.tracker_file)}

...
async def create_node(request: CreateNodeRequest):
    ...
    if result["success"]:
        ...
        raise HTTPException(
            status_code=400,
            detail=result.get("error", "Failed to create node")
        )
...
async def update_node(request: UpdateNodeRequest):
    ...
    if result["success"]:
        ...
        raise HTTPException(
            status_code=400,
            detail=result.get("error", "Failed to update node")
        )
...
async def create_relationship(request: CreateRelationshipRequest):
    ...
    Returns:
        OperationResponse with success status
    """
    logger.info(f"POST /create_relationship: {request.from_entity_type}→{request.relationship_type}→{request.to_entity_type}")
...
async def update_relationship(request: UpdateRelationshipRequest):
    ...
    Returns:
        OperationResponse with success status
    """
    logger.info(f"POST /update_relationship: {request.from_entity_type}→{request.relationship_type}→{request.to_entity_type}")

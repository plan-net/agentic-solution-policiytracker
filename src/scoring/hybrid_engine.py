...
class HybridScoringEngine:
    ...
    async def score_document_hybrid(self, document: ProcessedContent) -> ScoringResult:
        ...
        try:
            logger.debug("Starting hybrid scoring", document_id=document.id, llm_enabled=self.llm_enabled)
            ...
            if isinstance(insights, Exception):
                logger.warning("LLM insights failed, using fallback", 
                              document_id=document.id, error=str(insights))
            ...
            for i, score in enumerate(semantic_scores):
                if isinstance(score, Exception):
                    logger.warning("Individual semantic scoring failed", 
            ...
            logger.error("Hybrid scoring failed, falling back to rule-based", 
    ...
    def _create_hybrid_result(
        ...
        for dim_name, rule_dim_score in rule_based_result.dimension_scores.items():
            ...
            if semantic_score:
                ...
                rule_weight = 0.3
                final_score = (semantic_score.semantic_score * semantic_weight + 
                ...
                enhanced_justification = self._combine_justifications(
                    rule_dim_score.justification,
                    semantic_score.reasoning
                )
                
                # Enhanced evidence combining both sources
                enhanced_evidence = list(set(
                    rule_dim_score.evidence_snippets + semantic_score.key_factors
                ))[:3]  # Keep max 3 unique pieces of evidence
                
            else:
                # Fallback to rule-based score
                final_score = rule_dim_score.score
                enhanced_justification = rule_dim_score.justification + " (LLM analysis unavailable)"
        ...
        enhanced_justification = self._create_enhanced_justification(
            rule_based_result.overall_justification,
            insights,
            hybrid_master_score
        )
    ...
    def _create_enhanced_justification(
        ...
        if insights.confidence_score > 0.5:
            ...
            if insights.regulatory_indicators:
                llm_additions.append(f"regulatory indicators identified: {len(insights.regulatory_indicators)}")
    ...
    def _calculate_enhanced_confidence(
        ...
        high_confidence_semantic_scores: int
    ) -> float:

...
class DirectImpactScorer(DimensionScorer):
    ...
    def score(self, document: ProcessedContent) -> DimensionScore:
        ...
        # High-impact regulatory language
            ...
            regulatory_keywords = ["must comply", "required", "mandate", "penalty", "enforcement", "violation"]
        ...
        justification = "; ".join(justification_parts) if justification_parts else "Minimal direct relevance detected"
...
class IndustryRelevanceScorer(DimensionScorer):
    ...
    def score(self, document: ProcessedContent) -> DimensionScore:
        ...
        # Adjacent industry terms
            ...
            adjacent_industries = ["retail", "consumer goods", "technology", "digital", "marketplace"]
            ...
            if adjacent_matches >= 2:
                ...
                justification_parts.append(f"Adjacent industry relevance: {adjacent_matches} matches")
        ...
        justification = "; ".join(justification_parts) if justification_parts else "No industry relevance detected"
...
class GeographicRelevanceScorer(DimensionScorer):
    ...
    def score(self, document: ProcessedContent) -> DimensionScore:
        ...
        if primary_matches >= 2:
            ...
            evidence.extend(self.extract_evidence(document.raw_text, primary_markets + secondary_markets, 2))
        ...
        justification = "; ".join(justification_parts) if justification_parts else "No geographic relevance detected"
...
class TemporalUrgencyScorer(DimensionScorer):
    ...
    def score(self, document: ProcessedContent) -> DimensionScore:
        ...
        justification = "; ".join(justification_parts) if justification_parts else "No urgency indicators detected"
...
class StrategicAlignmentScorer(DimensionScorer):
    ...
    def score(self, document: ProcessedContent) -> DimensionScore:
        ...
        # Fallback to general strategic keywords
            ...
            strategic_keywords = ["innovation", "growth", "expansion", "transformation", "efficiency", "customer", "market"]
            keyword_matches = sum(1 for kw in strategic_keywords if kw.lower() in text)
            
            if keyword_matches >= 3:
                score = min(59, 45 + (keyword_matches * 3))
                justification_parts.append(f"General strategic relevance: {keyword_matches} keywords")
                ...
                justification_parts.append(f"Minimal strategic relevance: {keyword_matches} keywords")
                evidence.extend(self.extract_evidence(document.raw_text, strategic_keywords, 1))
        
        justification = "; ".join(justification_parts) if justification_parts else "No strategic relevance detected"

...
class ConfidenceCalculator:
    ...
    def calculate_confidence(
        ...
        keyword_density_factor = total_evidence / max_possible_evidence if max_possible_evidence > 0 else 0.0
        
        # Score variance factor (lower variance = higher confidence)
        scores = [score.score for score in dimension_scores.values()]
        if len(scores) > 1:
            score_std = statistics.stdev(scores)
            score_variance_factor = max(0.0, 1.0 - (score_std / 50))  # Normalize by max possible std
        else:
        ...
        evidence_quality_factor = min(total_evidence_chars / 1500, 1.0)  # Normalize by expected length

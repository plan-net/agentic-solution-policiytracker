...
class DocumentProcessor:
    ...
    def _extract_document_info(self, blob_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            doc_id = hashlib.sha256(content.encode() if isinstance(content, str) else content).hexdigest()[:16]
            ...
            logger.error(f"Failed to extract document info", error=str(e))
    ...
    def _extract_text(self, doc_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_extension == "pdf":
                ...
                text = doc_info["content"] if isinstance(doc_info["content"], str) else doc_info["content"].decode()
            else:
            ...
            logger.error(f"Failed to extract text", document_id=doc_info.get("document_id"), error=str(e))
    ...
    def _chunk_document(self, doc: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(0, len(text), self.chunk_size - self.chunk_overlap):
                ...
                chunk_id = f"{doc['document_id']}_chunk_{i // (self.chunk_size - self.chunk_overlap)}"
            ...
            logger.error(f"Failed to chunk document", document_id=doc.get("document_id"), error=str(e))

    ...
    class DocumentProcessorActor:
        ...
        def _read_document(self, doc_path: Path) -> str:
            ...
            try:
                ...
                logger.warning(f"Actor {self.actor_id}: UTF-8 failed for {doc_path}, trying latin-1")
        ...
        async def process_document(self, doc_path_str: str) -> Dict[str, any]:
            ...
            try:
                ...
                self.tracker.mark_processed(doc_path, {
                ...
                logger.info(f"Actor {self.actor_id}: Successfully processed {doc_path.name} "
        ...
        for result in results:
            if result["status"] == "success":
                ...
                if self.processing_stats["failed"] <= 3:
                    logger.error(f"Document processing failed: {result['path']} - {result.get('error', 'Unknown error')}")
...
""")
        ...
        logger.info(f"Ray parallel processing completed in {total_time:.2f}s",
                   total=len(documents),
    ...
    async def _process_with_ray(self, documents: List[Path], tracer) -> List[Dict[str, any]]:
        ...
        if tracer:
            await tracer.markdown(f"🚀 **Using {len(successful_actors)} Ray actors** for parallel processing")

...
from src.llm.models import (
    LLMProvider, DocumentInsight, SemanticScore, 
...
class LLMService:
    ...
    async def score_dimension_semantic(
        ...
        async with langfuse_client.trace(
            ...
            async def primary_scoring():
                return await self.primary_client.score_dimension(text, dimension, context, rule_based_score)
            
            async def fallback_scoring():
                return await self.fallback_client.score_dimension(text, dimension, context, rule_based_score)
    ...
    async def batch_analyze_documents(
        self, 
        documents_with_context: List[tuple[str, Dict[str, Any]]]
    ...
    async def batch_score_dimensions(
        self,
        scoring_requests: List[Dict[str, Any]]
    ) -> List[SemanticScore]:
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_fallback": True,
            "mock_calls": self.fallback_client.call_count if hasattr(self.fallback_client, 'call_count') else 0
        }

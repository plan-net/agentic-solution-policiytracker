...
from src.llm.models import (
    DocumentInsight, SemanticScore, TopicAnalysis, 
    LLMReportInsights, LLMProvider
)
...
class LangChainLLMService:
    ...
    async def _execute_with_fallback(
        ...
        try:
            ...
            if self.primary_llm:
                ...
                logger.debug(f"Successfully executed {operation_name} with {self.current_provider.value}")
                return result
    ...
    async def analyze_document(self, text: str, context: Dict[str, Any]) -> DocumentInsight:
        ...
        async def execute_analysis(llm: BaseChatModel) -> DocumentInsight:
            ...
            async with await langfuse_client.trace(
                ...
                metadata={"provider": self.current_provider.value, "operation": "document_analysis"}
            ) as trace:
    ...
    async def score_dimension_semantic(
        ...
        mock_response = SemanticScore(
            provider=LLMProvider.MOCK,
            semantic_score=max(0.0, min(100.0, rule_based_score + (hash(text + dimension) % 21 - 10))),
            confidence=0.6,
    ...
    async def analyze_topics_batch(self, documents: List[str], context: Dict[str, Any]) -> List[TopicAnalysis]:
    ...
    async def generate_report_insights(
        ...
        report_prompt = ChatPromptTemplate.from_messages([
            ("system", """Generate executive insights for this political monitoring report.

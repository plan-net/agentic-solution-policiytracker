...
"""
    ...
    async def analyze_topics(self, documents: List[str]) -> List[TopicAnalysis]:
        ...
        combined_text = "\n\n---DOCUMENT SEPARATOR---\n\n".join(documents[:8])  # Limit for token efficiency
...
"""
        ...
        if not response.success:
            ...
            return LLMReportInsights(
                ...
                strategic_recommendations=["Review high-priority documents", "Monitor regulatory developments"],
        ...
        try:
            ...
            return LLMReportInsights(
                executive_summary=response.content[:300] + "..." if len(response.content) > 300 else response.content,

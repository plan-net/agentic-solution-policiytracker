...
class AzureCheckpointSaver(BaseCheckpointSaver):
    ...
    async def aget(
        ...
        *,
        ...
        try:
            ...
            checkpoint_data = await self.azure_client.download_json(self.container_type, checkpoint_key)
            
            if not checkpoint_data:
                logger.debug("Checkpoint not found", thread_id=thread_id, checkpoint_id=checkpoint_id)
                return None
    ...
    async def aput_writes(
        ...
        try:
            ...
            if not thread_id or not checkpoint_id:
                logger.warning("Missing thread_id or checkpoint_id for writes", 
            ...
            logger.error("Failed to save writes", 
                        thread_id=thread_id,
                        checkpoint_id=checkpoint_id,
                        task_id=task_id,
                        error=str(e))
    ...
    async def _get_latest_checkpoint_id(self, thread_id: str) -> Optional[str]:
        ...
        try:
            ...
            logger.error("Failed to get latest checkpoint ID", 
        ...
        for key, value in checkpoint.items():
            try:
                ...
                try:
                    ...
                    logger.warning("Failed to serialize checkpoint value", 
                                 key=key, error=str(e))
                    serialized[key] = None
        ...
        for key, value in checkpoint_data.items():
            if isinstance(value, dict) and value.get("_type") == "pickle":
                try:
                    ...
                    logger.warning("Failed to deserialize pickled value", 
                                 key=key, error=str(e))
    ...
    async def cleanup_old_checkpoints(self, thread_id: str, keep_last: int = 10) -> int:
        ...
        try:
            ...
            blobs_to_delete = checkpoint_blobs[:-keep_last] if len(checkpoint_blobs) > keep_last else []
            ...
            for blob in blobs_to_delete:
                try:
                    ...
                    writes_blobs = await self.azure_client.list_blobs(self.container_type, prefix=writes_prefix)
            ...
            logger.error("Failed to cleanup old checkpoints", 
                        thread_id=thread_id, error=str(e))
            return 0
    
    # Synchronous methods (required by base class)
    def get(self, config: Dict[str, Any], **kwargs) -> Optional[Tuple[Dict[str, Any], Dict[str, Any], Optional[str]]]:
        """Synchronous get method (not recommended for Azure Storage)."""
        import asyncio
        return asyncio.run(self.aget(config, **kwargs))
    
    def put(self, config: Dict[str, Any], checkpoint: Dict[str, Any], metadata: Dict[str, Any], new_versions: Dict[str, Union[str, int]]) -> Dict[str, Any]:

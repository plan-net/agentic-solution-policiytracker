...
class AzureStorageClient:
    ...
    def __init__(self):
        ...
        logger.info("Azure Storage client initialized", account_name=account_name)
    ...
    async def ensure_container_exists(self, container_type: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "Failed to create container", container_name=container_name, error=str(e)
                )
                return False
    ...
    async def download_blob(self, container_type: str, blob_name: str) -> Optional[bytes]:
        ...
        try:
            ...
            logger.debug(
                "Downloaded blob", container=container_name, blob_name=blob_name, size=len(data)
            )
            return data

        except ResourceNotFoundError:
            logger.warning("Blob not found", container_type=container_type, blob_name=blob_name)
            return None
        except Exception as e:
            logger.error(
    ...
    async def download_blob_text(
        self, container_type: str, blob_name: str, encoding: str = "utf-8"
    ) -> Optional[str]:
    ...
    async def list_blobs(self, container_type: str, prefix: str = "") -> List[str]:
        ...
        try:
            ...
            logger.debug(
                "Listed blobs", container=container_name, prefix=prefix, count=len(blob_names)
            )
            return blob_names

        except Exception as e:
            logger.error(
                "Failed to list blobs", container_type=container_type, prefix=prefix, error=str(e)
            )
            return []
    ...
    async def get_blob_properties(self, container_type: str, blob_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning(
                "Blob not found for properties", container_type=container_type, blob_name=blob_name
            )
            return None
        except Exception as e:
            logger.error(
    ...
    async def delete_blob(self, container_type: str, blob_name: str) -> bool:
        ...
        try:
            ...
            logger.info("Deleted blob", container=container_name, blob_name=blob_name)
            return True

        except ResourceNotFoundError:
            logger.warning(
                "Blob not found for deletion", container_type=container_type, blob_name=blob_name
            )
    ...
    async def upload_file(
        self, container_type: str, blob_name: str, file_path: str, **kwargs
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to upload file", file_path=file_path, blob_name=blob_name, error=str(e)
            )
            return False
    ...
    async def download_to_file(self, container_type: str, blob_name: str, file_path: str) -> bool:
        ...
        try:
            ...
            if data:
                ...
                logger.info("Downloaded blob to file", blob_name=blob_name, file_path=file_path)
                return True
            return False

        except Exception as e:
            logger.error(
                "Failed to download to file", blob_name=blob_name, file_path=file_path, error=str(e)
            )
            return False

    async def cleanup_old_blobs(
        self, container_type: str, prefix: str, max_age_days: int = 7
    ) -> int:
...
async def save_report(
    job_id: str, report_name: str, content: str, format_type: str = "markdown"
) -> bool:
    """Save generated report."""
    blob_name = f"jobs/{job_id}/{report_name}.{format_type}"
    metadata = {"job_id": job_id, "format": format_type}
    return await get_azure_storage_client().upload_blob(
        "reports", blob_name, content.encode("utf-8"), metadata
    )

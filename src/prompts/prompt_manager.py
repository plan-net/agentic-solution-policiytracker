...
class PromptManager:
    ...
    async def get_prompt(
        ...
        try:
            ...
            logger.warning("Failed to load prompt from Langfuse", 
        ...
        try:
            ...
            logger.info("Prompt loaded from local file (fallback)", 
                       prompt_name=name, total_fallbacks=self.fallback_count)
            return self._substitute_variables(prompt_text, variables or {})
            
        except Exception as e:
            logger.error("Failed to load prompt from local file", 
                        prompt_name=name, error=str(e))
    ...
    async def preload_prompts(self) -> None:
        ...
        for prompt_file in prompt_files:
            ...
            try:
                ...
                logger.warning("Failed to preload prompt", 
                              prompt_name=prompt_name, error=str(e))

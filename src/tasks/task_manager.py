...
class TaskManager:
    ...
    async def process_documents_parallel(
        ...
        try:
            logger.info("Starting parallel document processing", file_count=len(file_paths))
    ...
    async def score_documents_parallel(
        ...
        try:
            logger.info("Starting parallel document scoring", document_count=len(documents))
    ...
    async def _process_small_batch(
        ...
        logger.info(
            "Small batch processing completed", processed=len(results), total=len(file_paths)
        )
            ...
            for task in ready_tasks:
                try:
                    ...
                    if progress_callback:
                        ...
                        await progress_callback(len(all_results), len(file_paths), progress)
    ...
    async def _score_small_batch(
        ...
        while tasks:
            ...
            ready_tasks, remaining_tasks = ray.wait(
                tasks, num_returns=min(len(tasks), 5), timeout=30.0
            )
        ...
        logger.info("Small batch scoring completed", scored=len(results), total=len(documents))
    ...
    async def _score_large_batch(
        ...
        while batch_tasks:
            # Wait for batch completion
            ready_tasks, remaining_tasks = ray.wait(
                batch_tasks, num_returns=min(len(batch_tasks), 3), timeout=60.0
            )
            ...
            for task in ready_tasks:
                try:
                    ...
                    if progress_callback:
                        progress = completed_batches / len(batches) * 100
                        await progress_callback(len(all_results), len(documents), progress)

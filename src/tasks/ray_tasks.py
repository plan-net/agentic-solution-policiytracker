...
def process_document_task(file_path: str, doc_id: str) -> ProcessedContent:
    ...
    try:
        ...
        logger.error(
            "Document processing task failed", file_path=file_path, doc_id=doc_id, error=str(e)
        )
        raise
...
def score_document_task(content: ProcessedContent, context: Dict[str, Any]) -> ScoringResult:
    ...
    try:
        ...
        logger.error("Document scoring task failed", document_id=content.id, error=str(e))
        raise
...
def batch_processing_task(file_paths: List[str], start_index: int) -> List[ProcessedContent]:
    ...
    try:
        logger.info(
            "Starting batch processing task", batch_size=len(file_paths), start_index=start_index
        )
        ...
        for i, file_path in enumerate(file_paths):
            try:
                ...
                logger.warning(
                    "Failed to process document in batch", file_path=file_path, error=str(e)
                )
                # Continue with other documents

        logger.info("Batch processing completed", processed=len(results), total=len(file_paths))
...
def batch_scoring_task(
    documents: List[ProcessedContent], context: Dict[str, Any]
) -> List[ScoringResult]:
    ...
    try:
        ...
        for document in documents:
            try:
                ...
                logger.warning(
                    "Failed to score document in batch", document_id=document.id, error=str(e)
                )
                # Continue with other documents

        logger.info("Batch scoring completed", scored=len(results), total=len(documents))
...
def create_scoring_batches(
    documents: List[ProcessedContent], batch_size: int = 20
) -> List[List[ProcessedContent]]:

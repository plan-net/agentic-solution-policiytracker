...
class ReportGenerator:
    ...
    async def prepare_report_data(
        ...
        try:
            ...
            summary = ReportSummary(
                ...
                processing_time_minutes=metrics.get("processing_statistics", {}).get("total_time_ms", 0) / 60000,
                confidence_overview={
            ...
            logger.error("Failed to prepare report data", 
                        job_id=job_id, 
                        error=str(e))
            raise
    ...
    def validate_report_data(self, report_data: ReportData) -> List[str]:
        ...
        if total_in_queues != len(report_data.all_results):
            issues.append(f"Priority queue count mismatch: {total_in_queues} vs {len(report_data.all_results)}")
